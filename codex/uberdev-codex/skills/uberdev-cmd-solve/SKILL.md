...
- `--auto` → enable `--dangerously-skip-permissions` on the spawned agent (post-#241 follow-up: historically mapped to `--permission-mode auto`, but auto-mode silently refuses some agent tools — notably Search — both inside and outside cmux, so the middle tier was dead weight; AUTO now resolves to the same strict bypass as SKIP). The trade-off is broad: dangerous tools no longer prompt. Use only when the issue is unattended-friendly (e.g., a /turbo batch or a /solve invocation you'll let run to completion). Else `SOLVE_AUTO=1` env var, else `solve_auto: true` in `.codex/uberdev.local.md`.
