---
name: uberdev-cmd-status
description: "Use when the user wants to read-only census of everything UberDev has in flight — /solve + /turbo claims, /goal cycle state, /review-pr reservations, the /merge lock, and per-run agent liveness — with the runtime root each store was actually read from and a concrete re-entry command per row. Never writes. Invokable explicitly as $uberdev-cmd-status. Original description: Read-only census of everything UberDev has in flight — /solve + /turbo claims, /goal cycle state, /review-pr reservations, the /merge lock, and per-run agent liveness — with the runtime root each store was actually read from and a concrete re-entry command per row. Never writes."
---

# Codex bridge — read first

This skill was ported from a Claude Code slash command (`/status`). On Codex:
...
```bash
. "${PLUGIN_ROOT:-${CODEX_HOME:-$HOME/.codex}/plugins/uberdev-codex}/lib/status.sh" && uberdev_status_render
```
