...
PY
   ...
   **Workflow mandate:** the fence above validated
   `[ -f "${PLUGIN_ROOT:-${CODEX_HOME:-$HOME/.codex}/plugins/uberdev-codex}/skills/review-fleet/workflow.js" ]`. Relay the JSON
   between `WORKFLOW_ARGS_BEGIN`/`WORKFLOW_ARGS_END` **verbatim** (DR-2 — no
   LLM-composed handoffs) into:

   ```
   Workflow({scriptPath: "${PLUGIN_ROOT:-${CODEX_HOME:-$HOME/.codex}/plugins/uberdev-codex}/skills/review-fleet/workflow.js"}, <the JSON between the markers>)
   ```
