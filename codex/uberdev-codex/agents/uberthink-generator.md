...
Under `skills/uberthink-pipeline/workflow.js` the orchestration reads a `StructuredOutput` return: `islandIndex` (the integer K from your dispatch), `persona` (your persona name), `candidateCount` (the integer number of mechanisms you wrote) and `outPath`. Keep emitting the YAML handle above as well — it is what the No-Workflow fallback orchestrator reads.

**Wave-1 frame availability.** In the Workflow path, Wave 1 fires in the SAME burst as the three remaining Wave-0 lenses, so at wave 1 only `frame.md` is guaranteed to exist. `teardown.md`, `prior-art.md` and `constraints.md` are being written concurrently: read whichever already exist, never block on them, and never invent their contents. Your dispatch prompt says which are guaranteed. Wave-2 (gap-targeted) dispatches always have all four.
