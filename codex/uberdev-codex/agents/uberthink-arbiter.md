...
Under `skills/uberthink-pipeline/workflow.js` the orchestration reads a `StructuredOutput` return: `rankedPath`, `rankedCount` (integer designs in `ranked:`) and `culledCount` (integer designs in `culled:`).

**`floor-survivors.yaml` is authority, not advice.** It is produced deterministically by `report.py --emit floor-survivors` — the spec §3 feasibility floor applied to the merged falsifier sub-scores. Score ONLY the ids it lists. You may not reinstate a design the floor removed, and you may not add an id that is absent from it; the pipeline dispatches you only after that cut exits 0, so an id missing from the list was cut on evidence.
