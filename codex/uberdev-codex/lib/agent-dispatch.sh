...
_uberdev_agent_validate_file_path() {
  local run_dir="$1" requested="$2" mode="$3"
  python3 -I -c '
import os, stat, sys
run_dir, requested, mode = sys.argv[1:]
...
try:
    if os.path.commonpath((run_dir, resolved)) != run_dir:
        raise SystemExit(2)
except ValueError:
    raise SystemExit(2)
...
uberdev_agent_dispatch() {
  ...
  local prompt_file result_file status_file state_dir manifest catalog decision lease event_json rc handle terminal_event
  ...
  prompt_file="$(_uberdev_agent_validate_file_path "$run_dir" "$prompt_requested" input)" || { _uberdev_agent_error 'unsafe prompt path'; return 2; }
  result_file="$(_uberdev_agent_validate_file_path "$run_dir" "$result_requested" output)" || { _uberdev_agent_error 'unsafe result path'; return 2; }
  status_file="$(_uberdev_agent_validate_file_path "$run_dir" "$status_requested" output)" || { _uberdev_agent_error 'unsafe status path'; return 2; }
  ...
  if ! PYTHONPATH= PYTHONHOME= uberdev_semaphore_set_handle "$lease" "$handle" "$status_file"; then
  ...
  if [ -n "$terminal_event" ]; then
    ...
    PYTHONPATH= PYTHONHOME= uberdev_semaphore_release "$lease" >/dev/null 2>&1 || return 2
  fi
  DISPATCH_RC=0
  return 0
}
