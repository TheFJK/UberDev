...
- `plugins/uberdev/hooks/` — `SessionStart` / `UserPromptSubmit` / `SessionEnd` / `PreCompact` event handlers. Claude Code wiring is `hooks.json`: every entry routes through the `run-hook.cmd` polyglot so the same scripts run on Windows, and every entry declares `"shell": "bash"` so the interpreter is not chosen per host (#461 — an undeclared handler falls back to PowerShell on a Windows box without Git Bash, which parses the quoted command path as an expression and the hook never loads, silently; declared, that host throws a named error instead). Because bash parses `run-hook.cmd` on that path, the hook sources must stay LF — `/.gitattributes` pins `plugins/uberdev/hooks/** text eol=lf`. Cursor wiring is `hooks-cursor.json` (executes the bash scripts directly — POSIX hosts only; **Cursor on Windows is unsupported**, pinned by `tests/docs-accuracy.test.sh`).
