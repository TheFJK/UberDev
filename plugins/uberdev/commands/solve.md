---
description: "Spawn an autonomous agent per GitHub issue via a platform-aware dispatch backend (claude-bg / wezterm / background / codex; auto-selected per host), with auto-triage and tier-appropriate workflow. Monitor via `claude agents` (claude-bg), visible panes (wezterm), or PID/log/result files (background/codex). Accepts multiple issue numbers; dispatches one agent per issue. Override the backend with `--backend=<name>`."
