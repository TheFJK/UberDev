...
`triage-scout` returns **one** `status` field covering **two** independent probes (`agents/triage-scout.md`, "Failure modes"): a failed `gh label list` and a failed `gh search issues` both surface as `status: DONE_WITH_CONCERNS`, and the failed *search* reports `duplicates: []` — byte-identical to a search that ran and matched nothing. The status alone therefore never says which probe failed. Classify the return here, before Phase 3, and carry the classification into the Phase 5 gate:

- **Duplicate state PROVEN.** `status: DONE` (the scout's contract routes every probe failure to a non-`DONE` status, so `DONE` is the affirmative that the search completed), **or** `duplicates` is non-empty (which is itself proof the search ran). `duplicates` may now be read at face value.
- **Duplicate state UNKNOWN.** Any non-`DONE` status that does not affirm the search completed — `status: BLOCKED`; a `status: DONE_WITH_CONCERNS` whose `summary` names the duplicate search as failed, rate-limited or truncated; a `status: DONE_WITH_CONCERNS` whose `summary` is silent about which probe degraded — or no parseable YAML return at all. Empty-but-unproven is **not** empty. Re-dispatch `uberdev:triage-scout` **exactly once** with the identical brief: a rate limit or a transient 502 usually clears on the second look, and a `DONE` retry re-enters the gate with a decidable verdict at zero interaction cost. Exactly once, never a loop. If the retry is still unproven, the duplicate state is `UNKNOWN` for Phase 5, which halts on it. Never round `UNKNOWN` down to "no duplicates found".

When the classification is PROVEN but `valid_labels` is empty (`gh label list` failed while the search succeeded), the main thread proceeds without a `--label` flag and records the omission as a `degradation:` line in the Phase 7 result block. It does **not** halt and it does **not** ask — labels are an enrichment, whereas the duplicate search is the gate's own input.
...
Evaluate one predicate against the Phase 2 `triage-scout` return, using that phase's PROVEN/UNKNOWN classification: does `duplicates` **provably** contain no entry whose `state` is `open`? The gate fails **closed** — only a proven absence clears it, so the predicate has three outcomes, two of which halt.
...
- **Full-text duplicate search** — `gh search issues`, include closed (regression evidence) — never `gh issue list --limit 30`. If the search itself cannot run, that is `UNKNOWN`, not "no duplicates": retry once, then halt at Phase 5. An unproven `duplicates: []` is never a clean bill of health.
