...
**Behavior vs `/solve`:**
- **trivial / small tiers:** identical to `/solve` except the child agent dispatches in the auto-mode branch (`AUTO_MODE=1`). Both `/solve` and `/turbo` route trivial/small through `uberdev:finish-branch` → `/uberdev:review-pr` (Phase 1 reviewer fanout runs post-push; `--turbo` forwarded under `/turbo`).
