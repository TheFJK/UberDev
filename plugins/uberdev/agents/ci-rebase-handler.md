...
The lease lives in `commands/review-pr.md` Step 6c.4 ROUTE. It is recorded here
so a future reader does not "restore" it to this agent:
...
5. **Conflict handling:** **leave the rebase IN PROGRESS** — do NOT `git rebase --abort`, and do NOT resolve the conflicts yourself. Return `status: CONFLICT` with `conflict_count`. The controller enumerates the conflicted paths from its OWN unmerged-path enumeration (`code_fixer_contract.py list-ci-unmerged-paths`, reached through `review_fleet_unmerged_paths`, which covers every porcelain unmerged pair — `DD`, `AU`, `UD`, `UA`, `DU`, `AA`, `UU` — and not the both-modified pair alone, because an add/add rebase conflict records `AA` and would otherwise enumerate as the empty set) and dispatches one `uberdev:conflict-resolver` per file as the `ci-conflicts` stage. Taking that set from your return instead would let the agent whose failure produced the conflict choose which files its successors may touch.
