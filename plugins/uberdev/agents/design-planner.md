...
The task ids, the wave numbers and the `Owns` values above are **literal on purpose**. The controller parses `### Task <digit>:`, `**Wave:** wave-<digit>` and `**Owns (file allowlist):** <comma-separated paths on the SAME line>` out of the plan with real regexes, and a bracketed placeholder in any of those three slots parses to nothing. Task *titles* may stay angle-bracket placeholders; ids, waves and `Owns` values may not. Keep the `Owns` paths on the label's own line: that is the form this template shows, and it is the one the `plan-reviewer` reads. The parser is the only authority on what else it will accept — read `OWNS_RE` and `read_bullet_list` in `lib/turbox-fleet.sh` rather than any restatement of that grammar, here or in another card.

One more parse hazard, and it is the one the `unwaved` / `unowned` counters cannot see — `duplicate_labels` is the counter that does, and Step 5 below requires it empty: a **second** `**Wave:**` or `**Owns (file allowlist):**` line anywhere inside a task — including one you write into a step body to show a worker the line it must emit — silently replaces the task's declared value. The two label regexes in `lib/turbox-fleet.sh` (`WAVE_RE` and `OWNS_RE`, cited by name because a line number here rots on the next edit to that file) match against each line *stripped of leading whitespace*, so indenting the restatement does not protect it, and the last match inside a task wins. Both counters still report empty, because the task does end up waved and owned — just not with the values you wrote. `plan-reviewer` Check 2 then reviews the allowlist you declared while TB3 `wave-disjoint` enforces the one that overwrote it, so a wave whose declared `Owns` lists are strictly disjoint can still be refused `rc 3` at dispatch and launch zero children. Never restate either label inside a step body; describe the line in prose instead.
...
5. **Parseability — run the shipped parser; do not eyeball it.** By inspection: every task heading is `### Task <digit>:`, every wave line is `**Wave:** wave-<digit>`, every `**Owns (file allowlist):**` carries its paths on the same line, and no step body restates either label. Then prove it with the parser the controller itself dispatches from:
   ...
   The JSON it prints must show `"unwaved":[]`, `"unowned":[]`, `"duplicate_labels":[]`, and a `task_count` equal to the number of `### Task <digit>:` sections you wrote — the same integer you return in `task_count` below. `duplicate_labels` is the one that catches a restated label: it names every task that carried a **second** `**Wave:**` or `**Owns …:**` line, which overwrites the declared value without moving `unwaved` or `unowned`. Read the `tasks` array as well, to see WHICH value survived. Any discrepancy means the controller will strand or misroute those tasks; fix the plan and re-run until every part of this holds.
