...
`/fix` trades `/turbox`'s eight-or-more **sequential** design rungs for two —
three when the review comes back red. **What it drops is design, not
diligence:** the change is still committed by explicit path, still reviewed by
an independent agent, still validated by the canonical boundary, still run
against the project's full test suite, and still parked as a PR nobody merges.
Skipping the review, or opening the PR without running the suite, does not make
`/fix` faster — it makes it dishonest.

**When the issue is bigger than this lane, say so and stop.** Phase 2 names the
one return that says it. `references/rationale.md` carries the lane comparison
and the reasoning behind each give-up.
