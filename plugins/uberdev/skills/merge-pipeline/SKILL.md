...
for CLEAR_ISSUE_NUM in "${CLOSED_ISSUES[@]}"; do
  if gh issue edit "$CLEAR_ISSUE_NUM" --remove-label "uberdev:active" >/dev/null 2>&1; then
    _uberdev_audit_emit uberdev_active_label_cleared \
      "{\"issue\":$CLEAR_ISSUE_NUM,\"pr\":$PR,\"reason\":\"merge\"}" || true
  fi
  # B5: also clear the @me assignee so the dispatcher's "Assigned to me" view
  # does not accumulate closed-and-merged issues. Fail-soft (same semantics as
  # the remove-label call): the issue may already have been unassigned by hand,
  # or the assignee may differ from @me (e.g. teammate triaged the issue
  # before the /solve dispatcher ran). Either way the operator does not want
  # an audit-noisy failure on a UI-cleanup pass.
