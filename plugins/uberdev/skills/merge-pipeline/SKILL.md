...
| `REVIEW_PR_DEFERRED_CRITICAL_REGEX` | `^severity=critical-deferred count=[0-9]+$` — the trailer suffix the `review-pr` instrument carries on a YELLOW (deferred-critical) trail, and its only defined non-empty value. An empty suffix is GREEN; a RED run emits no anchor commit at all. Sole producer: `review_fleet_trailer_suffix` in `lib/review-fleet-args.sh`, which derives it from the same durable state on every YELLOW path. Read here for exactly the reason the premerge token is: `review-pr-verdict.json` lives under gitignored `.uberdev/`, so a YELLOW that existed only there is unreadable on every clone but the producer's. | Phase 1.4 PATH_2 `merge-base-identity-fence-v1` (b.5), for instrument `review-pr`; gated against `ACCEPT_CRITICAL_DEFERRED_FLAG` |
...
| `TRUST_ANCHOR_ENUM` | `reviewDecision_approved`, `uberdev_review_trail`, `uberdev_premerge_trail`, `bypass_with_waiver` (deprecated; never emitted post-v0.17.0) | Phase 1.4 (audit-log `gate_pass.data.trust_anchor`) |
| `GATE_FAIL_REASON_ENUM` | **trust-resolution reasons** (PATH_1 / PATH_2): `review_decision_not_approved`, `trust_trail_missing`, `trust_trail_stale_sha`, `trust_trail_label_missing`, `trust_trail_trailer_missing`, `trust_trail_json_missing`, `trust_trail_agent_invalid_input`, `trust_trail_json_sha_mismatch`. **Pre-condition gate reasons** (Step 1.4 pre-flight, evaluated before trust resolution): `pr_state_not_open`, `is_draft`, `ci_red`, `merge_state_blocked`. **Infrastructure failure reasons**: `pr_view_unreachable` — emitted when the pr-view projection lib call exits non-zero (Step 1.4); the PR is skipped, the queue continues, and a discovery-gh-failed audit event is emitted alongside. `pr_base_unresolvable` — emitted by Step 1.4.7 and Step 3.0 when the resolve_pr_base helper cannot resolve the PR's OWN base ref (`baseRefName` absent/null in the projection, a `BRANCH_NAME_REGEX` reject, or `refs/remotes/origin/<base>` still absent after the fetch); that PR is skipped and the queue continues. **Never fall back to the repo-global integration branch on this path** — that silent substitution IS #437. `pr_base_parent_skipped` — emitted by Step 1.4.6 and Step 3.2 when the PR's base is another candidate's head and that parent did NOT land in this run (it gate-failed in Phase 1, or failed during Phase 3); merging the child anyway would land it into its parent's branch and emit a merge-executed row /goal reads as convergence, while the integration branch receives nothing. (Only the backticked lowercase tokens in this cell are counted by M37's row-count assertion, so helper and variable names here are deliberately left unbackticked.) Total 15 members — the eight trust-resolution reasons are subject to M37's enum-row count assertion; the four pre-condition reasons are emitted by Step 1.4 pre-flight gates that fire regardless of trust path; the three infrastructure failure reasons are emitted when a lib call itself fails, when a PR's own base cannot be resolved, and when a stack parent did not land. | Phase 1.4, Phase 1.4.6 / 1.4.7, Phase 3.0 and Phase 3.2 (audit-log `gate_fail.data.reason`) |
...
| `DISCOVERY_WIRE_LIMIT` | `200` (the `--limit` on `discover_multi`'s `gh pr list`; `UBERDEV_DISCOVERY_WIRE_LIMIT` in `lib/discover.sh`, env-overridable). Matches `lib/goal-state.sh`'s existing `--limit 200` PR enumerations. **Explicit because the base filter is now client-side (#437):** `gh pr list` defaults to `--limit 30`, and while the filter was on the wire that window was spent entirely on integration-branch PRs. Off the wire, the same window spans EVERY open PR in the repo and the base filter runs on whatever survived truncation — so in a repo with >30 open PRs, eligible candidates are dropped before the filter sees them and `/merge` reports a short or empty set: the same false-convergence signal `/goal` consumes. Widening the query without widening the window converts a server-side bound into a client-side one. | Step 1.2.5 (`DISCOVERY_FILTER`'s wire query) |
| `DISCOVERY_FILTER` | `discover_multi` in `lib/discover.sh` (R1 in-process filter — `gh pr list --state open --limit <DISCOVERY_WIRE_LIMIT> --search 'draft:false' --json number,title,headRefOid,headRefName,baseRefName,isDraft,createdAt,reviewDecision,labels,body,author,headRepositoryOwner --jq '[.[] \| select(.isDraft==false)]'`; the belt-and-suspenders `isDraft==false` filter runs inside gh's process so no external jq pipe is ever created). **The base-branch filter is client-side, not on the wire (#437):** `_uberdev_discover_stack_filter` applies `$integration_branch` as the ROOT of a **transitive** reachability closure (a candidate is in scope when its `baseRefName` is the root OR the `headRefName` of another in-scope candidate, iterated to fixpoint). The retired `--base "$integration_branch"` flag was an EXACT server-side match, so a PR stacked on another PR's head returned zero candidates and `/merge` exited 0 reporting "nothing to merge" — the false-convergence signal `/goal` consumes. A server-side exact match cannot be repaired downstream because the stacked rows never arrive. **The explicit `--limit` is part of the same fix, not decoration:** the wire window must exceed what the retired server-side filter used to bound, because the base filter now runs AFTER truncation — see `DISCOVERY_WIRE_LIMIT`. A returned row count equal to the window emits a stderr saturation breadcrumb. | Step 1.2.5 (multi-discover dispatch — runs after Step 1.2 integration_branch resolution); also referenced by `--all` for one canonical filter shared by both modes (Q4). Sourced by SKILL.md, never invoked inline. |
...
| `CI_OUTCOME_ENUM` | `green`, `green_after_fix`, `skipped_no_checks`, `halted`, `loop_cap_exhausted` | `commands/review-pr.md` Phase 3 terminal audit + Step 7 trust-signal predicate. **`loop_cap_exhausted` is LIVE again since #383** — produced past 6c.4 ROUTE, which the Workflow-native `ci-fix` stage now reaches. **`green_after_fix` is LIVE since #400** — both reachable green terminals (6c.1 PROBE's fast path and 6c.2 MONITOR) derive it via `review_fleet_ci_green_outcome`, which reads `fix_pushes` out of `ci-loop-state.json`, so a PR whose head an autopilot rewrote now records `green_after_fix` rather than being indistinguishable from one that was always green. Both remain accepted by the Step 7 trust predicate — the distinction carries information to trust-trail consumers, it does not gate. Members annotated DORMANT between #381 and #383 (RFC 0002 amendments 2026-08-05 and 2026-08-06) were never removed, so consumers validating historical audit JSON kept working throughout. |
...
The function runs `gh pr list ... --jq '[.[] | select(.isDraft==false)]'` in-process (R1 — the belt-and-suspenders `isDraft==false` filter executes inside gh's Go process on the parsed object before serialising stdout, so no external `jq` pipe is ever created and gh's spinner / progress bytes cannot break a downstream parse). The candidate array is the input set for Phase 1.4 (per-PR trust gate fanout).

**Stacked PRs are in scope (#437).** The base-branch filter is applied CLIENT-SIDE by `_uberdev_discover_stack_filter`, not by a `--base` flag on the wire. `$integration_branch` is the ROOT of a **transitive** reachability closure: a candidate is in scope when its `baseRefName` is the root OR is the `headRefName` of another in-scope candidate, iterated to fixpoint (so a 3-deep stack roots correctly, not just the first hop). The retired `gh pr list --base "$integration_branch"` was an exact server-side match, which returned ZERO candidates for a PR stacked on another PR's head — `/merge` then took Step 1.7's clean-exit-0 path and reported "nothing to merge", which is the false-convergence signal `/goal` consumes. Client-side is the only place the fix can live: a server-side exact match never returns the stacked rows at all. **The only call site of `discover_multi`** is this step — both bare-mode (multi-discover) and `--all` route through this single dispatch point so the two modes share one canonical filter (Q4). On gh-or-jq failure, the function emits a `discovery_gh_failed` audit event with `data.step="1.2.5"`, prints a `warning:` breadcrumb to stderr, and returns the literal `'[]'` to stdout — Step 1.7's clean-exit-0 contract still applies (the empty candidate set produces a clean-exit-0 run-summary block).
...
a. **Resolve the instrument SET from the labels.** At least one member of `TRUST_LABEL_ENUM` is present on the PR — else gate_fail with `data.reason="trust_trail_label_missing"`. Every matched member contributes its instrument to `LABEL_INSTRUMENTS` ⊆ `TRUST_INSTRUMENT_ENUM`; a label is not merely a presence bit, and the resolution is a SET rather than a first-arm-wins race. Match on whole label names, never on substrings — `uberdev-approved-with-concerns` contains `uberdev-approved` and a substring test would mis-resolve it:
   ...
   # `premerge-approved`, and `commands/review-pr.md` leaves that one alone on
   # its GREEN and YELLOW arms (only its RED arm clears every TRUST_LABEL_ENUM
   # member, and a RED run emits no trail to compose with) -- so the documented
   # compose order (`/premerge`, then `/review-pr <stack-pr>`; README and RFC
   # 0021 A7) leaves BOTH labels on the PR. A `case` stopping at the first
   # matching arm resolved whichever instrument the author listed first, then
   # disagreed with the newest anchor's trailer at (b.5): a hard refusal.
   LABEL_INSTRUMENTS=
   ...
   **The set widens WHICH instrument may be resolved, never WHETHER one must be.** An empty `LABEL_INSTRUMENTS` is the `trust_trail_label_missing` gate_fail above, and a non-empty one is not yet a pass: (b.5) requires the trailer's own instrument to be a MEMBER of this set, so a PR wearing only `premerge-approved` still refuses a `review-pr` trailer and one wearing only `uberdev-approved` still refuses a `premerge` trailer. What the set buys is exactly that a dual-labelled PR is resolved by the trailer it actually carries instead of by arm order.
   ...
   The with-concerns row is not new tolerance: `commands/review-pr.md` removes the GREEN label from a deferred-critical PR and adds the with-concerns one instead, so a bare equality against the GREEN literal refused every such trail at (a) — before `--accept-critical-deferred` could be consulted at (c). The strictness that decides a deferred-critical trail did not move onto the label, and it is not the evaluator's Phase 2.5 gate alone: that gate only runs when `review-pr-verdict.json` is discoverable, which — the artifact being gitignored — it is not on most checkouts. What holds the tier is the commit-body suffix gate stated at (b.5), which reads ` severity=critical-deferred count=N` off the immutable anchor and demands `--accept-critical-deferred` before (c) ever dispatches, with the evaluator's gate reaching the identical verdict wherever the artifact does exist. Admitting this label WITHOUT that second gate would be genuine new tolerance.
...
a.5. **Resolve the TRUST HEAD before (b) reads anything (issue #364).** `/goal` guarantees the mandatory version bump at its serialized landing lane — the only point in a run where the next version is unambiguous — which means a `chore(release): vX.Y.Z` commit legitimately lands on the PR head AFTER `/review-pr` anchored the trail. Read the trailer off `headRefOid` unconditionally and every `/goal` PR is unmergeable forever: (b) fails `trust_trail_trailer_missing` and, were the trailer to survive, (c) would see a non-empty cumulative diff and return `STALE`. So resolve the OID that (b) and (c) are evaluated against, once, via the helper — never by pattern-matching the subject inline:
   ...
   When `RELEASE_ANCHOR == "tolerated"`, the eventual `gate_pass` carries `data.release_anchor="inert_release_commit"` and `data.release_anchor_version="<X.Y.Z>"` alongside a `data.trust_anchor` this step does not touch: the anchor is whatever (c)'s PASS mapping derives from `$TRUST_INSTRUMENT` — `uberdev_review_trail` for `review-pr`, `uberdev_premerge_trail` for `premerge` — because tolerating a release commit changes WHICH COMMIT the trail is read off and never WHICH INSTRUMENT wrote it. (a.5) runs before (b.5) has parsed the trailer, so it cannot know the instrument and must not name one. **Naming `uberdev_review_trail` here unconditionally was wrong on the COMMON path, not on an edge case:** `/premerge` emits its trust anchor in Phase `5-trail` and then commits `chore(release): vX.Y.Z` on top in Phase `5a`, so an inert release commit sitting over a `premerge` trail is the ordinary shape of a parked stack PR — exactly the shape `RELEASE_ANCHOR=tolerated` exists to read through, and exactly the shape whose audit row a hardcoded `review-pr` anchor would mislabel, defeating the tier-distinguishability (c) is built to provide. This is a D15 field-level extension — `TRUST_ANCHOR_ENUM` and `GATE_FAIL_REASON_ENUM` are unchanged. Omit both `release_anchor` fields otherwise.

b. The `TRUST_HEAD` commit body contains a trailer matching `^Reviewed-by: uberdev/review-pr@([a-f0-9]{40})$` (extract via `git log -1 --format=%B "$TRUST_HEAD" | grep -E ...`; see `REVIEW_PR_TRAILER_PREFIX` constant) — else gate_fail with `data.reason="trust_trail_trailer_missing"`. With no tolerated release anchor `TRUST_HEAD == headRefOid`, so this is the most-recent commit exactly as before. The quoted form is the `review-pr` instrument's instantiation of `TRUST_TRAILER_REGEX`, not the whole domain: a `premerge` trailer satisfies (b) on the same terms, (b.5) below is the binding that actually reads either one, and the `gate=green attempt=NN` requirement that the `premerge` instrument adds to (b) is stated immediately after that fence.
...
b.5. **Resolve the BASE half of the trail before (c) dispatches (#440).** The `Reviewed-by:` trailer names one endpoint of the reviewed delta. On its own it lets a GREEN survive `gh pr edit <N> --base <other-branch>`: no push occurs, so `headRefOid` cannot move; the trailer is bytes inside an immutable commit; the `uberdev-approved` label is a bare literal with no payload. Every trust artifact stays byte-identical, the evaluator's three structural primitives all return their PASS values, and this skill maps that to `gate_pass` while an entirely unreviewed delta lands. So read the second endpoint off the SAME commit body, and answer the base question HERE — the evaluator has neither `gh` nor `git merge-tree`, and that boundary is not relaxed for this feature:
   ...
   TRUST_TRAILER_LINE="$(git log -1 --format=%B "$TRUST_HEAD" \
                         | grep -E '^Reviewed-by: uberdev/(review-pr|premerge)@[a-f0-9]{40}( .*)?$')" || true
   ...
   # The instrument is DATA, so it is parsed rather than assumed. Strip the fixed
   # `uberdev/` prefix once, then split on the FIRST `@`: everything before it is
   # the instrument, everything after it up to the first space is the SHA, and the
   # remainder is the instrument's own suffix. An empty line yields an empty
   # instrument, which the domain check below refuses -- absence must reach the
   # classifier as a value, never as a silently-defaulted `review-pr`.
   TRUST_TRAILER_BODY="${TRUST_TRAILER_LINE#Reviewed-by: uberdev/}"
   TRUST_INSTRUMENT="${TRUST_TRAILER_BODY%%@*}"
   TRAILER_SHA="${TRUST_TRAILER_BODY#*@}"
   TRAILER_SHA="${TRAILER_SHA%% *}"
   TRAILER_SUFFIX="${TRUST_TRAILER_BODY#*@$TRAILER_SHA}"
   ...
   TRUST_BASE_LINE="$(git log -1 --format=%B "$TRUST_HEAD" \
                      | grep -E '^Reviewed-base: uberdev/(review-pr|premerge)@[a-f0-9]{40} ref=.+$')" || true
   TRUST_BASE_LINE="${TRUST_BASE_LINE%%$'\n'*}"
   REVIEWED_BASE_SHA=null; REVIEWED_BASE_REF=null
   if [ -n "$TRUST_BASE_LINE" ]; then
     TRUST_BASE_BODY="${TRUST_BASE_LINE#Reviewed-base: uberdev/}"
     TRUST_BASE_INSTRUMENT="${TRUST_BASE_BODY%%@*}"
     REVIEWED_BASE_SHA="${TRUST_BASE_BODY#*@}"
     REVIEWED_BASE_SHA="${REVIEWED_BASE_SHA%% *}"
     REVIEWED_BASE_REF="${TRUST_BASE_LINE#* ref=}"
     # Both halves of the trail must name the SAME instrument. A body carrying one
     # command's head endpoint and another's base endpoint is not a trail either
     # command emitted, and treating it as one is the forgery this check exists for.
     [ "$TRUST_BASE_INSTRUMENT" = "$TRUST_INSTRUMENT" ] \
       || BASE_IDENTITY_REFUSAL=trust_trail_trailer_missing
   fi
   ...
   # REVIEW_PR_DEFERRED_CRITICAL_REGEX, classified in the same shell and for
   # the same reason. `/review-pr` writes ` severity=critical-deferred count=N`
   # on EVERY YELLOW anchor and an empty suffix on every GREEN one
   # (`review_fleet_trailer_suffix`; RED emits no anchor at all), so the commit
   # body is a complete and immutable record of the tier. Discarding it left the
   # deferred-critical gate reachable ONLY through the gitignored
   # `review-pr-verdict.json`: off the producing checkout discovery returns
   # `audit_state=absent`, the evaluator skips its Phase 2.5 gate by design, and
   # the YELLOW lands with --accept-critical-deferred never consulted. The FLAG
   # is command-line state this fence may not assume, so classify here and let
   # the caller acknowledge -- the same split as PREMERGE_GATE_ATTEMPT. An
   # unrecognised suffix REFUSES rather than reading as GREEN: the producer's
   # domain is closed, so a third form is corruption, not a new tier.
   REVIEW_PR_DEFERRED_CRITICAL=
   if [ -z "$TRAILER_CONTRACT_REFUSAL" ] && [ "$TRUST_INSTRUMENT" = review-pr ]; then
     case "$TRAILER_SUFFIX" in
       "") ;;
       "severity=critical-deferred count="*)
         REVIEW_PR_DEFERRED_CRITICAL="${TRAILER_SUFFIX##*=}"
         case "$REVIEW_PR_DEFERRED_CRITICAL" in
           "" | *[!0-9]*)
             REVIEW_PR_DEFERRED_CRITICAL=
             TRAILER_CONTRACT_REFUSAL=trust_trail_trailer_missing
             ;;
   ...
   printf 'LABEL_INSTRUMENTS=%s\nTRAILER_CONTRACT_REFUSAL=%s\nPREMERGE_GATE_ATTEMPT=%s\nREVIEW_PR_DEFERRED_CRITICAL=%s\n' \
     "$LABEL_INSTRUMENTS" "$TRAILER_CONTRACT_REFUSAL" "$PREMERGE_GATE_ATTEMPT" "$REVIEW_PR_DEFERRED_CRITICAL"
   ...
   **For instrument `premerge`, the trailer must carry a green gate token.** `$TRAILER_SUFFIX` must match `PREMERGE_GATE_TOKEN_REGEX` (`^gate=green attempt=[0-9]{2}$`) — else `gate_fail` with `data.reason="trust_trail_trailer_missing"`. The fence runs this test where `TRAILER_SUFFIX` is bound, as a whole-string `case` glob rather than a `[[ =~ ]]` the zsh Bash tool would evaluate with `BASH_REMATCH` unset, folds the verdict into the same `TRAILER_CONTRACT_REFUSAL`, and captures the attempt number in `PREMERGE_GATE_ATTEMPT` for (d)'s corroborator lookup. Testing this regex against a `$TRAILER_SUFFIX` bound in some other shell is not a stricter reading of the check — it is a check that fails for EVERY premerge trail, which kills the tier as silently as the vacuous pass above opens it. This is the payload that keeps the premerge tier from resting on two bare literals, and it lives in the commit body rather than in `gate-NN.json` because `.uberdev/` is gitignored and a clone that never ran the producer has no such file. For instrument `review-pr` the suffix is optional in SHAPE only, never in meaning: an empty suffix is GREEN, the deferred-critical ` severity=critical-deferred count=N` form is YELLOW, and the paragraph immediately below gates on which of the two the anchor actually carries.

   **For instrument `review-pr`, a deferred-critical suffix requires the acknowledgement flag — and the commit body is the only place that fact survives a clone.** `$TRAILER_SUFFIX` must match `REVIEW_PR_DEFERRED_CRITICAL_REGEX` or be empty; the fence classifies it where the value is bound, prints the parsed count as `REVIEW_PR_DEFERRED_CRITICAL` (empty for GREEN), and folds an unrecognised third form into the same `TRAILER_CONTRACT_REFUSAL`. The acknowledgement itself is the CALLER's, because the flag is command-line state the fence may not assume — the same split by which `PREMERGE_GATE_ATTEMPT` is captured here and consumed in (d). **When the printed count is a positive integer and `ACCEPT_CRITICAL_DEFERRED_FLAG` was not passed, emit `gate_fail` with `data.reason="trust_trail_stale_sha"`, skip this PR before (c) dispatches, and continue the queue** — the same reason, and the same softness, that the evaluator's own critical-deferred gate produces from `phase2_5_critical_count > 0`, so the two paths agree and no `GATE_FAIL_REASON_ENUM` member is added. Read the count off the fence's printed output like every other value it resolves; a prose test over a name bound in some other shell compares empty to empty and admits exactly what it exists to refuse. **This is not a duplicate of the evaluator's gate — it is the only copy that survives distribution.** `review-pr-verdict.json` lives under gitignored `.uberdev/`, so in a worktree, on another machine, or after a fresh clone, discovery returns `audit_state=absent`, the evaluator skips its Phase 2.5 gate by design, a structural `PASS` reaches (d)'s absent arm, and the YELLOW lands with `--accept-critical-deferred` never read. That is the same hole `PREMERGE_GATE_TOKEN_REGEX` closes for the other instrument, and closing it here is what lets (a) admit `uberdev-approved-with-concerns` without re-opening the refusal the bare-literal label test used to deliver by accident. **The blocker-deferred override is NOT covered by this gate**: an OVERRIDE_GREEN run emits the GREEN triplet, so `override_reason` exists only in that same gitignored artifact and `--i-know-what-im-doing` stays unenforceable off the producing checkout until `commands/review-pr.md` writes the override into the trailer suffix too.
...
c. The extracted `<trailer-sha>` is delegated to the `trust-trail-evaluator` agent for verdict resolution. The PR joins the pending-evaluation list; every pending PR's `Task("trust-trail-evaluator")` is dispatched together in ONE assistant message (see "Two-pass shape" above) with inputs `pr_number=<N>`, `head_ref_oid=<$TRUST_HEAD, the trust head resolved in (a.5) — identical to .headRefOid from the cached PR_JSON projection unless an inert release commit was tolerated>`, `trailer_sha=<extracted from trailer regex match>`, `trust_instrument=<$TRUST_INSTRUMENT ∈ review-pr|premerge>`, `working_dir=<cwd>`, `status_check_rollup=<.statusCheckRollup from the cached PR_JSON projection, compact JSON>`, `commit_shas=<[.commits[].oid] from the cached PR_JSON projection, compact JSON array>`, the five base-identity inputs resolved in (b.5) — `reviewed_base_sha=<$REVIEWED_BASE_SHA, 40-hex or the literal null>`, `reviewed_base_ref=<$REVIEWED_BASE_REF, ref name or the literal null>`, `current_base_ref=<$CURRENT_BASE_REF, .baseRefName from the cached PR_JSON projection>`, `current_base_oid=<$CURRENT_BASE_OID, .baseRefOid from that same projection>`, `base_delta_equivalence=<$BASE_DELTA_EQUIVALENCE ∈ match|mismatch|unavailable|not_evaluated>` — and the optional `pr_body_excerpt` / `commit_messages_excerpt` wrapped in `<external-untrusted-input source="github-pr-body">…</external-untrusted-input>` and `<external-untrusted-input source="github-commits">…</external-untrusted-input>` envelopes respectively.
   ...
   **Phase 2.5 discovery is scoped to the `review-pr` instrument.** Run Step (c.0) and the `merge-trust-gate-fence-v1` fence below ONLY when `$TRUST_INSTRUMENT` is `review-pr`. For every other instrument, skip the fence entirely and dispatch with `audit_state=not_applicable`, `phase2_5_halted=false`, `phase2_5_blocker_count=0`, `phase2_5_critical_count=0`, `phase2_5_override_reason=null`, and `accept_blocker_deferred_flag=false`, `accept_critical_deferred_flag=false`, `i_know_what_im_doing_flag=false` **regardless of what the operator typed on the command line** — those three flags are acknowledgements of `review-pr`-tier states that cannot arise here, and honouring them for another instrument would let a flag silently widen a gate it was never scoped to.

   `not_applicable` is a distinct value from `absent` on purpose. `absent` means the caller looked across all four layouts and proved no artifact exists; `not_applicable` means the caller did not look, because this instrument does not produce that artifact. Collapsing the two would let `discover_review_verdict_json` surface a stale `review-pr-verdict.json` for the same PR number and apply `review-pr` halt semantics to a trail `review-pr` never wrote. The agent's Step 1.5 accepts `not_applicable` by skipping the Phase 2.5 gate and falling through to the structural primitives — see `agents/trust-trail-evaluator.md`.
...
On all four sub-conditions met: emit `gate_pass` with the `data.trust_anchor` that (c)'s PASS mapping derives from `$TRUST_INSTRUMENT` — `uberdev_review_trail` for `review-pr`, `uberdev_premerge_trail` for `premerge`. **This line SUMMARISES (c) and (d); it is never a second mapping.** It is also the copy an orchestrator reads when it skips the rows above, so re-hardcoding one instrument here stamps every premerge landing `uberdev_review_trail` and destroys exactly the tier distinguishability `TRUST_ANCHOR_ENUM` exists to provide — the same defect (a.5) already carries a paragraph about, one summary line further down. Proceed.
...
### Step 1.4.6 — Prune orphaned stack children (Half-2 safety gate, #437)

Step 1.2.5's stack closure admits candidates whose base is another candidate's head. That closure ran over the **discovered** set. Step 1.4 then removes candidates from it. Re-run the closure over the set that actually **survived**:
...
# Emits gate_fail reason="pr_base_parent_skipped" for every dropped child.
survivors=$(prune_orphaned_candidates "$integration_branch" "$survivors")
```

**Why this gate exists.** Before #437 every survivor shared one base, so this could not happen. Now it can, and the failure is silent and severe: PR-A gate-fails on an ordinary reason (`ci_red`, `trust_trail_missing`), PR-B is stacked on A's head and is green, so PR-B reaches Step 3.2 and `gh pr merge <B>` merges it into **GitHub's recorded base — `fix/a`, not the integration branch**. That emits `merge_executed`, which is exactly the event `/goal`'s `uberdev_goal_read_merge_result` selects, so the run reports converged and PR-B's `Closes #N` closes the issue — while the integration branch never received a line of the code. A green child of a skipped parent is NOT mergeable; it is blocked on a dependency that did not land.

The prune is a **reachability closure over the survivors, not a one-hop parent check**: dropping PR-B must also drop anything stacked on PR-B. `prune_orphaned_candidates` reuses the same fixpoint reduction as Step 1.2.5's discovery filter, so the two cannot drift. Each dropped candidate gets `gate_fail` with `data.reason="pr_base_parent_skipped"` (∈ `GATE_FAIL_REASON_ENUM`) — never a silent drop, and never a retarget onto `<integration_branch>`.

**This is a gate, not the whole guard.** Step 3.2 re-checks the same invariant per iteration (a parent can also fail *during* Phase 3, after this gate passed) — see "Step 3.2 — Clean-merge path".

**This one helper fails OPEN, and that is not an exception to the fail-closed rule — it is the same rule.** `resolve_pr_base` fails CLOSED because its failure mode is "probe against the wrong ref", and the safe answer there is to merge nothing. `prune_orphaned_candidates` only ever REMOVES candidates, so its failure mode is the mirror image: a jq error that returned `[]` would convert a transient tool fault into "nothing to merge" — manufacturing the exact false-convergence signal `/goal` consumes that this whole issue exists to kill. It therefore returns the candidate set UNCHANGED on an unevaluable closure, emits one stderr breadcrumb, and accuses no PR (no `gate_fail`). Nothing unsafe reaches a merge that way, because Step 3.2's parent-landed guard is evaluated per iteration and is the check that must never be skipped.

### Step 1.4.7 — Bind and prove each surviving PR's base (Phase-1/2 binding)

**Where the Phase-1 base binding comes from.** Steps 1.5 and 2.2 take `<PR_BASE>` from the `baseRefName` field of the Step 1.4 `pr_view_projection` for that PR — the same projection instant the trust gate was decided against, so the planning inputs and the gate agree on one view of the PR. This does **not** contradict Step 3.0(a)'s "re-read per iteration, never a value cached from Phase 1" rule. That rule governs Phase 3, where PRs land mid-run and GitHub auto-retargets the bases of their children; Phases 1 and 2 are planning phases that run once, before anything has landed, so the Phase-1 projection is the correct binding there and Phase 3 re-reads it fresh.

**The Phase-1/2 ranges are `origin/`-qualified too.** `git fetch` writes `refs/remotes/` only, and for a stacked PR the base is another PR's branch which has **no local ref at all** in the merge runner's clone. A bare `<PR_BASE>..<head>` there does not silently give a wrong answer — it exits **128** with `fatal: ambiguous argument`, for exactly the PRs #437 exists to admit. The `origin/`-qualified invariant of Step 3.0 therefore applies to **every** site that names a base ref, Phase-1 and Phase-2 sites included, not only the Phase-3 write sites.

So, once per run, immediately after Step 1.4.6 and before the matrix: fetch every DISTINCT `<PR_BASE>` in the surviving set in ONE call (`git fetch origin "<integration_branch>" "<base-1>" "<base-2>" …`), then resolve each PR's base through the SAME lib helper Phase 3 uses:

```bash
# Phase-1 binding. Same helper, same fail-closed contract as Step 3.0(c).
PR_BASE_REF=$(resolve_pr_base "$PR" "<PR_BASE>") || {
  # gate_fail reason="pr_base_unresolvable" already emitted by the helper.
  continue   # skip THIS PR; the queue continues
}
```

**A failed range is never read as a result.** If a range command exits non-zero anyway (a restricted `remote.origin.fetch` refspec, a base deleted between projection and fetch), its EMPTY stdout must NOT be consumed: empty output from `git diff --name-only` is indistinguishable from "these PRs share no files", and empty output from `git rev-list --count` is not the number 0. Emit an `error` audit event with `data.reason="base_range_failed"` and degrade in the SAFE direction — Step 1.5 records the pair as overlapping (forcing sequential ordering, the conservative choice), Step 2.2 skips that PR rather than dispatching `merge-strategy-decider` on empty inputs.

### Step 1.5 — Compute file-overlap matrix (Q1 same-file degradation pre-compute)

For every pair of in-scope PRs, run `git diff --name-only origin/<PR_BASE>..<pr-N-head>` — each PR's OWN `baseRefName`, `origin/`-qualified per Step 1.4.7, not the repo-global branch — and intersect path sets. **Why per-PR (#437):** for a PR stacked on another PR's head, `<integration_branch>..<head>` also enumerates the parent PR's touched files, so the matrix reports a phantom overlap between the two (they "overlap" by construction, always) and forces spurious sequential ordering. This input is advisory — it cannot corrupt a merge — but it is wrong for exactly the PRs Step 1.2.5's stack closure exists to admit. Pairs with non-empty intersection are flagged for sequential ordering in Phase 2 (PR-A first, then re-probe PR-B against new tip). Distinct-file pairs remain eligible for parallel conflict-resolve in Phase 3.
...
1. **Hard dependencies** (highest priority): a PR-B base ref equal to PR-A head ref → PR-A must land before PR-B. Also parse `body` for `Depends on #([0-9]+)` (whitelist regex) and add those edges. topo-sort the resulting graph. **On cycle: emit one stderr line citing the full cycle path, drop the cycle's edges, fall through to createdAt order for the cycle members, and continue.** Never halt — the user can re-issue dependency edits and re-run /merge if the auto-break ordering proves wrong. **No audit event is emitted for cycle-break by design** — the stderr line is the canonical surface (cycle-break is a planning-phase decision, not a per-PR outcome; `AUDIT_EVENT_ENUM` intentionally has no `cycle_detected` member). Implementers MUST NOT add an audit event here without spec-level changes.
   ...
   > **The base-ref edge was UNREACHABLE before #437 — the rule itself was never dead code.** Be precise about which half was broken. The `Depends on #N` body parse has always worked: nothing filtered it, and every multi-PR run topo-sorted and cycle-broke on those edges. The *base-ref* edge could not fire, for two independent reasons that between them covered every mode. In multi-discover mode the retired `--base "$integration_branch"` wire filter guaranteed every survivor shared one base, so edge (a) would have needed some PR-A whose `headRefName` equalled the integration branch — a PR from `main` into `main`. In single-PR mode the step is skipped outright (see the "Skip Step 2.1" line below). Step 1.2.5's client-side stack closure makes the edge reachable by admitting stacked candidates. **The sort needs no change:** it is source-agnostic — it consumes edges without caring which source produced them — so a newly-populated edge set flows through unchanged. Do not "simplify" this edge away on the grounds that nothing produced it.
...
For each PR in the in-scope set, dispatch a `merge-strategy-decider` agent. **Every range below is anchored on `origin/<PR_BASE>` — that PR's own `baseRefName`, `origin/`-qualified (Step 1.4.7), not the repo-global `<integration_branch>` and never a bare local ref (#437 + #303).** For a stacked PR, `<integration_branch>..<head>` enumerates the PARENT PR's commits as well, so `commit_count` is inflated and `conventional_commit_ratio` / `wip_marker_present` are computed over commits belonging to a different PR entirely — the strategy would be decided on someone else's diff; and a BARE `<PR_BASE>..<head>` is worse still, because a stacked PR's base has no local ref in the merge runner's clone and the range exits 128 instead of answering. The chosen strategy is real (squash vs merge), not advisory, so an empty `commit_count` from a failed range must never reach the agent — see Step 1.4.7's `base_range_failed` degradation. Inputs per PR: `pr_number`, `commit_count` (from `git rev-list --count origin/<PR_BASE>..<head_ref_oid>`), `conventional_commit_ratio` (from a regex pass over `git log origin/<PR_BASE>..<head_ref_oid> --format=%s` matching `^(feat|fix|chore|refactor|test|docs)(\(.+\))?:`), `wip_marker_present` (from a regex pass over the same log matching `WIP_MESSAGE_REGEX`), `divergence_commits` (`git rev-list --count <merge-base>..<head_ref_oid>`, where the merge-base is taken against `origin/<PR_BASE>`), `label_hint` (suffix of any `merge-strategy:<name>` label on the PR, advisory; null otherwise; wrapped in `<external-untrusted-input source="github-pr-label">…</external-untrusted-input>` envelope), `repo_convention` (recorded preference from `.claude/uberdev.local.md` `merge_strategy:` key, null if absent), `working_dir`.
...
### Step 3.0 — Per-iteration fetch + per-PR base resolve + lock heartbeat (stale-tip guard, #303; base-awareness, #437)

At the top of EACH landing iteration — not once per run — re-read this PR's own base, refresh the remote-tracking refs, and prove lock liveness.

**(a) Re-read `<PR_BASE>`.** Re-project this PR only (`pr_view_projection <N>`, the same call Step 1.4.5's green-path re-eval uses) and take `baseRefName` from the refreshed projection. `<PR_BASE>` is a per-PR placeholder the orchestrator carries for this iteration, exactly like `<headRefOid>` and `<headRefName>` — **not** a shell variable read from a Phase-1 fence. Fence-scoped shell state dies per fence (see `## Common Mistakes`), so a `PR_BASE=…` bound in the Step 1.4 fence would be an empty string here, and an empty base makes `git merge-tree --write-tree origin/ <sha>` resolve nothing and exit 1 — a phantom conflict again.

**Do NOT cache `<PR_BASE>` across iterations.** When PR-A merges, GitHub auto-retargets PR-B's base from PR-A's head to the integration branch, and the next `git fetch --prune` may delete the old ref. A value cached at Step 1.4 would make iteration N+1 probe a ref that no longer exists, gate-failing PR-B *after* its dependency successfully landed — a partially-landed stack, the worst available outcome. Step 3.0 already re-fetches per iteration for the twin "the tip moved" reason; the base re-read belongs in the same place.
...
When (and only when) `<PR_BASE>` differs from `<integration_branch>` — i.e. this PR is stacked — fetch its base as well. The extra fetch is conditional so a PR based on the integration branch produces byte-identical argv to pre-#437:

```bash
# Stacked PR only: also make the PR's OWN base ref present in refs/remotes/.
git fetch origin "<PR_BASE>"
```

`<integration_branch>` stays in the unconditional fetch: Phase 4's local sync still needs it, regardless of what any individual PR is based on.

**(c) Resolve and GUARD the base.** Before any probe, resolve the ref through the lib helper:
...
`resolve_pr_base` runs three checks and prints `origin/<PR_BASE>` only if all three pass: the base is present and non-null; it satisfies `BRANCH_NAME_REGEX` (`baseRefName` is GitHub-supplied text reaching `git` argv — the same surface Step 1.2 already gates on the global branch, and the same gate applies here); and `git rev-parse --verify -q refs/remotes/origin/<PR_BASE>` resolves. **Fail closed:** every failing arm emits `gate_fail` with `data.reason="pr_base_unresolvable"` (∈ `GATE_FAIL_REASON_ENUM`), prints nothing, and the PR is skipped. **Never fall back to `<integration_branch>`** — silently substituting the global branch for an unresolvable per-PR base is precisely the defect #437 fixes.

**Why the rev-parse guard is not optional.** `git merge-tree` exits **1** for a ref it cannot resolve — the SAME exit code it returns for a genuine conflict. A base that was never fetched is therefore byte-indistinguishable from a real conflict by exit code alone, and would re-create the phantom-conflict half of this bug through the back door. `git rev-parse --verify` is the only honest discriminator, and it must run BEFORE the probe. (A restricted `remote.origin.fetch` refspec — the single-branch-clone case — is exactly how the ref can still be missing after step (b).)

Then run the canonical lock-heartbeat touch snippet (see "Lock heartbeat protocol", touch site 4).

**Why per-iteration:** after PR-A lands server-side, the integration tip moves. Probing PR-B against the pre-A tip produces false-clean probes that surface only as server-side `gh pr merge` failures with NO conflict-resolve attempt (the conflict path never fires because the local probe lied). A fetch-failure (network blip) is non-fatal: emit one stderr warning + an `error` audit event with `data.reason="git_fetch_failed"`, and continue — the `--match-head-commit` TOCTOU guard and the server-side merge still protect correctness; the probe just degrades to the last-fetched tip. (A fetch failure that leaves `refs/remotes/origin/<PR_BASE>` absent is caught by (c)'s guard and gate-fails that PR; it is never silently probed against the global branch.)

**The fetch alone is NOT sufficient — and neither is the global branch.** Two separate invariants apply to **every site in this skill that names a base ref** — Phase-3 write sites AND the Phase-1/2 read sites of Steps 1.5 and 2.2 — and both are load-bearing:

1. **`origin/`-qualified, never the bare local ref (#303 / RFC 0012 §3.2).** `git fetch` updates `refs/remotes/` only — it never moves a LOCAL branch ref. Probing or basing a worktree on the bare local ref defeats the fetch entirely; that is the actual stale-tip bug. This invariant is unchanged by #437 and must not be regressed by it. **Do not read this rule as Phase-3-only:** a stacked PR's base has no local ref *at all* in the merge runner's clone, so a bare range at Step 1.5 / 2.2 exits 128 rather than answering — the qualification is what makes the planning phases work on stacked PRs, not a Phase-3 nicety.
2. **The PR's OWN base, never the repo-global one (#437).** Steps 3.1, 3.3.ii, 3.3.iii and 3.3.v(b) therefore MUST reference `origin/<PR_BASE>` — the ref `resolve_pr_base` returned this iteration. A PR stacked on another PR's head has a base that is not `<integration_branch>`, and using the global branch is wrong in BOTH directions: it invents conflicts that do not exist on the merge actually being performed (conflict-resolvers fan out at nothing), and it hides conflicts that do (the clean path fires `gh pr merge` at a PR that does not merge). Both directions are executed against real git in `tests/merge-discovery-resilience.test.sh` B26.

Phase 4 is the deliberate exception to (2): it is a LOCAL-sync phase that runs once after the whole queue, not per PR, so every `<integration_branch>` reference there stays global by design.

### Step 3.1 — Probe (D9, non-destructive)

Run `git merge-tree --write-tree origin/<PR_BASE> <headRefOid>` (the `origin/` remote-tracking ref for this PR's OWN base, resolved and proven present by Step 3.0(c) — NEVER a bare local ref, which Step 3.0's fetch does not move, and NEVER `origin/<integration_branch>` unless that IS this PR's base). Exit 0 = clean; exit 1 = conflicts. **Never** use `git merge --no-commit --no-ff` — `merge-tree` is the canonical non-destructive primitive (no working-tree mutation). On conflict, parse the "Conflicted file info" section to enumerate the conflicted file paths.

### Step 3.2 — Clean-merge path

**Parent-landed guard (#437) — evaluate BEFORE `gh pr merge`.** `gh pr merge <N>` merges into **GitHub's recorded base**, not into whatever ref this run has been probing. So if the `<PR_BASE>` re-read in Step 3.0(a) is not `<integration_branch>`, this PR is still stacked, and merging it now would land it into its parent's branch and emit a `merge_executed` row that `/goal` reads as convergence — while the integration branch receives nothing. Require that the parent landed **in this run**: there must be a `merge_executed` row in `.uberdev/audit.jsonl` for the candidate whose `headRefName` equals `<PR_BASE>`. If there is not, emit `gate_fail` with `data.reason="pr_base_parent_skipped"` (∈ `GATE_FAIL_REASON_ENUM`), skip this PR, and continue the queue.

This is deliberately redundant with Step 1.4.6: that gate catches a parent that failed **before** Phase 3, this one catches a parent that failed **during** it (probe conflict, test fail, `gh pr merge` error). Neither subsumes the other. Note the healthy path passes for free — when PR-A lands, GitHub auto-retargets PR-B's base to the integration branch, and Step 3.0(a)'s re-read picks that up on the next iteration, so `<PR_BASE> == <integration_branch>` and the guard is a no-op. **Never "fix" a still-stacked PR by passing `--base` to `gh pr merge` or by retargeting it** — the parent's code has not landed, so the child is not mergeable, full stop.

If probe was clean and the parent-landed guard passed: run `gh pr merge <N> --<strategy> --match-head-commit <headRefOid>`. The `--match-head-commit` flag is mandatory — it is the TOCTOU guard that fails fast if the PR HEAD moved between probe and merge.
...
ii. **Create scratch worktree (D10):** `git worktree add .claude/worktrees/merge-<run-id> origin/<PR_BASE>` where `<run-id> = $(date +%Y%m%d-%H%M%S)-$(git rev-parse --short HEAD)`. The base is the `origin/` remote-tracking ref for THIS PR's own base, resolved by Step 3.0(c) (detached at the fetched remote tip) — basing on a bare local ref would materialize conflicts against a stale tip and defeat the Step 3.0 fetch (#303), and basing on the repo-global branch would materialize conflicts against a ref this PR is not merging into (#437). Verify `.claude/worktrees/` is gitignored (per `using-git-worktrees`).
...
This is the critical invariant. All Task() calls for this PR's conflict set MUST be in ONE assistant turn — splitting across messages defeats parallelism (analogous to the dispatch-before-wait rule inside one `uberdev:post-impl-review` wave). Each Task() invokes `agents/conflict-resolver.md` with `file_path`, `pr_branch=<headRefName>`, `integration_branch=origin/<PR_BASE>`, `base_sha=<merge-base of origin/<PR_BASE> and <headRefOid>>`, `working_dir=<scratch worktree root>`.

**Both of those values are load-bearing, not cosmetic (#437).** The agent runs `git show <integration_branch>:<file_path>` to read the "theirs" side of every hunk (see `agents/conflict-resolver.md` Process step 2), so a wrong value there does not merely waste a dispatch — it feeds the resolver the wrong file and corrupts the resolution. It must be the SAME ref the scratch worktree in (ii) was based on: `origin/<PR_BASE>`, the PR's own base, `origin/`-qualified. `base_sha` names both of its operands for the same reason — an unqualified "merge-base" left the second operand implicit, and the only reading that used to make sense was the global branch.
...
(b) **STRATEGY-SWITCH** (`data.choice="strategy-switch"`) — switch strategy (e.g. `squash` ↔ `merge`), re-probe via `git merge-tree --write-tree origin/<PR_BASE> <headRefOid>` (same `origin/`-qualified, per-PR-base form as Step 3.1 — this is the fourth Phase-3 write site and it must not drift back to the global branch), re-resolve if the new probe reports conflicts. **Max 1 switch per PR per run.** Emits `agent_strategy_switch` audit event with `data.from`, `data.to`, `data.rationale`. On test pass after switch: proceed to push. On test fail after switch: fall through to (c).
...
- **Don't re-hardcode the instrument.** `Reviewed-by: uberdev/<instrument>@<sha>` carries the producing command as DATA; `TRUST_INSTRUMENT_ENUM` is the closed set and `TRUST_LABEL_ENUM` is the label side of the same fact. Re-inlining `review-pr` into a regex, or testing one label literal for presence instead of resolving the instrument from the table, restores the defect where a genuinely-reviewed stack is refused for having been reviewed by the wrong command. And do NOT introduce a third path: the generalised PATH_2 is the design, `commands/merge.md` states there is no third anchor, and Phase 1.4 must not name one.
- **Don't let a `premerge` trail borrow `review-pr`'s Phase 2.5 semantics.** `audit_state=not_applicable` means the caller never looked, and the three acknowledgement flags are forced `false` for that instrument. Mapping `not_applicable` onto `absent` re-opens the path where a stale `review-pr-verdict.json` for the same PR number is read as telemetry about a run `/review-pr` never performed. Equally, do NOT drop the `gate=green attempt=NN` token requirement: without it the premerge tier rests on two bare literals with no payload, which is the same hole `Reviewed-base:` was added to close. **And do not drop its `review-pr` twin, the ` severity=critical-deferred count=N` suffix gate at (b.5).** `review-pr-verdict.json` is gitignored, so off the producing checkout that suffix is the only evidence a trail is YELLOW; delete the gate and `--accept-critical-deferred` becomes unreachable in precisely the case it exists for — a deferred-critical trail landing from a worktree, another machine, or a fresh clone.
