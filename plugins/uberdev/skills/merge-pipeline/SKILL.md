...
| Auto-review returned `refused_non_green` (Phase 1.4.5; `outcome="refused_non_green"`) | `/review-pr` returned exit 2 (Phase 2 fanout crash / artifact-emission failure), OR advisory-timeout kill (≥ inherited `MERGE_TIMEOUT` cap), OR dispatch failure (plugin disabled). Emit `auto_review_returned` with `outcome: refused_non_green` and `duration_ms` = elapsed time at kill / dispatch failure; exclude PR; run-summary line; queue continues | continues |
