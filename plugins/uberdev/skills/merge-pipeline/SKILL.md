...
| `CI_OUTCOME_ENUM` | `green`, `green_after_fix`, `skipped_no_checks`, `halted`, `loop_cap_exhausted` | `commands/review-pr.md` Phase 3 terminal audit + Step 7 trust-signal predicate. **STILL DORMANT after #383 half one:** `green_after_fix` and `loop_cap_exhausted` are produced only past 6c.4 ROUTE, which no surviving transport can reach. #383 half one built the Workflow-native `ci-fix` stage that will reach it, but did NOT re-point `commands/review-pr.md` at it, so both members stay unreachable until the Phase 3 fence wiring lands. They are retained, not removed, so consumers validating historical audit JSON keep working (RFC 0002 amendments 2026-08-05 and 2026-08-06). |
