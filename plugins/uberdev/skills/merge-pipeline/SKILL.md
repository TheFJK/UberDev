...
c. The extracted `<trailer-sha>` is delegated to the `trust-trail-evaluator` agent for verdict resolution. Dispatch in a single-message `Task("trust-trail-evaluator")` with inputs `pr_number=<N>`, `head_ref_oid=<live gh pr view --json headRefOid>`, `trailer_sha=<extracted from trailer regex match>`, `working_dir=<cwd>`, and the optional `pr_body_excerpt` / `commit_messages_excerpt` wrapped in `<external-untrusted-input source="github-pr-body">…</external-untrusted-input>` and `<external-untrusted-input source="github-commits">…</external-untrusted-input>` envelopes respectively.
   ...
   **Phase 2.5 inputs (RFC 0002 §3.6, added v0.26.0).** Before dispatch, parse the Phase 2.5 block from the same `.uberdev/runs/<run-id>/review-pr-verdict.json` audit file inspected by sub-condition (d). First **discover the path** by mirroring sub-condition (d)'s glob enumeration (single source of truth — the prose below); then parse:
   ...
   #     (per `solve-pipeline/SKILL.md`: `.claude/worktrees/solve-issue-N/`).
   #   - `.worktrees/*/.uberdev/runs/*` — `using-git-worktrees` preferred
   #     hidden convention (per `using-git-worktrees/SKILL.md:45`).
   #   - `worktrees/*/.uberdev/runs/*` — `using-git-worktrees` alternate
   #     visible convention (per `using-git-worktrees/SKILL.md:149`).
   # The `~/.config/uberdev/worktrees/<project>/<branch>/.uberdev/runs/*`
   # global-fallback layout from `using-git-worktrees/SKILL.md:46` is NOT
   # globbed (it lives outside the project root and requires runtime $HOME
   # resolution — out of scope for /merge's path-relative discovery; tracked
   # for the writer-side path-anchoring follow-up). When /merge runs from
...
- **Globbing only `.uberdev/runs/*/review-pr-verdict.json` for sub-condition (c.0) / (d) and missing the worktree-mirror paths.** `/review-pr` writes its audit JSON relative to its CWD; when `/merge` runs from the main checkout but the PR was produced by ANY worktree-based flow (`/solve` and `/turbo` per `solve-pipeline/SKILL.md`, OR `subagent-driven-dev` / `executing-plans` / brainstorm-Phase-4 per the generic `using-git-worktrees/SKILL.md`), the audit JSON lives inside that worktree's gitignored `.uberdev/runs/` — invisible from the main-checkout's first-glob alone. Both Step (c.0) bash discovery and sub-condition (d) prose MUST glob the FULL worktree-mirror enumeration alongside the canonical path:
  ...
  - `.claude/worktrees/*/.uberdev/runs/*/review-pr-verdict.json` — `/solve` / `/turbo` convention (`solve-pipeline/SKILL.md`: `.claude/worktrees/solve-issue-N/`).
  - `.worktrees/*/.uberdev/runs/*/review-pr-verdict.json` — `using-git-worktrees` preferred hidden convention (`using-git-worktrees/SKILL.md:45`).
  - `worktrees/*/.uberdev/runs/*/review-pr-verdict.json` — `using-git-worktrees` alternate visible convention (`using-git-worktrees/SKILL.md:149`).

  The `~/.config/uberdev/worktrees/<project>/<branch>/.uberdev/runs/*` global-fallback layout (`using-git-worktrees/SKILL.md:46`) is intentionally NOT globbed — it lives outside the project root and would require runtime `$HOME` resolution; that case is deferred to the writer-side path-anchoring follow-up (anchor `/review-pr`'s artifact writer on the main-checkout root via `git rev-parse --show-toplevel`, mirroring `orchestrator/SKILL.md` line 67). The pre-fix bug short-circuited `trust-trail-evaluator` to `STALE` via `phase2_5_present=false` in its Step 1.5 (legacy-audit branch) and gated otherwise-valid trust trails on every worktree-produced PR. The RUN_ID_REGEX basename-of-dirname projection (D4/F8 path-traversal hardening) works identically on all four path layouts because the prefix segments are never concatenated from untrusted input — no security regression. Future worktree conventions added to `using-git-worktrees/SKILL.md` MUST be paired with a glob addition here AND a matching `M63.worktree-glob.*` test assertion (5-surface fan-out: compgen-OR-chain + for-loop + sub-condition (d) prose + this bullet + tests/merge.test.sh).
