...
| `CI_OUTCOME_ENUM` | `green`, `green_after_fix`, `skipped_no_checks`, `halted`, `loop_cap_exhausted` | `commands/review-pr.md` Phase 3 terminal audit + Step 7 trust-signal predicate. **LIVE again since #383:** `green_after_fix` and `loop_cap_exhausted` are produced past 6c.4 ROUTE, which the Workflow-native `ci-fix` stage now reaches. They were annotated DORMANT between #381 and #383 (RFC 0002 amendments 2026-08-05 and 2026-08-06) but were never removed, so consumers validating historical audit JSON kept working throughout. |
