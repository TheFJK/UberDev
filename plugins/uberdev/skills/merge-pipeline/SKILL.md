...
| `CI_OUTCOME_ENUM` | `green`, `green_after_fix`, `skipped_no_checks`, `halted`, `loop_cap_exhausted` | `commands/review-pr.md` Phase 3 terminal audit + Step 7 trust-signal predicate. **DORMANT since #381:** `green_after_fix` and `loop_cap_exhausted` are produced only past 6c.4 ROUTE, which no surviving transport can reach — they are retained, not removed, so consumers validating historical audit JSON keep working (RFC 0002 amendment 2026-08-05). |
