...
| `CI_OUTCOME_ENUM` | `green`, `green_after_fix`, `skipped_no_checks`, `halted`, `loop_cap_exhausted` | `commands/review-pr.md` Phase 3 terminal audit + Step 7 trust-signal predicate. **`loop_cap_exhausted` is LIVE again since #383** — produced past 6c.4 ROUTE, which the Workflow-native `ci-fix` stage now reaches. **`green_after_fix` remains DORMANT:** no fence assigns it, so a PR whose head the autopilot rewrote records `green`, identical to one that was always green. Both are accepted by the Step 7 trust predicate, so the gap costs consumers information, not safety. Members annotated DORMANT between #381 and #383 (RFC 0002 amendments 2026-08-05 and 2026-08-06) were never removed, so consumers validating historical audit JSON kept working throughout. |
