...
pr_base_branch: feat/parent      # branch new PRs target (`gh pr create --base`) AND the pre-push secret-scan range base; default unset -> the origin default branch, and no --base flag is emitted; env override: UBERDEV_PR_BASE_BRANCH
...
**`pr_base_branch` precedence (#439):** env var `UBERDEV_PR_BASE_BRANCH` > config file (this YAML) > the origin default branch (`refs/remotes/origin/HEAD`, then `origin/main`, then `origin/master`). Value is validated against `^[A-Za-z0-9._/-]{1,255}$`; anything else falls back non-fatally with a `regex_mismatch` warning. Resolution is deliberately EXPLICIT-FIRST: `/uberdev:finish-branch` runs while already standing on the feature branch, and "what did I branch from" is not recoverable from git — `git merge-base` answers a different question and will confirm `main` on a branch whose real parent is another PR. The resolved base has exactly two consumers and they must agree: the pre-push secret-scan diff range, and `gh pr create --base`. The first two tiers emit `--base`; the default tier does NOT, because the value it resolves to is already what `gh` targets on its own — leaving the unset path byte-identical to pre-#439 behaviour. Set this to a parent PR's branch to open a stacked (dependent) PR: it both retargets the new PR and keeps the parent's commits (and their secret-shaped test fixtures) out of the child's scan range. If the configured base resolves to no ref in the local checkout (a fresh stacked worktree that never fetched the parent, or a typo), `--base` is still passed to `gh` — GitHub has the branch even when this clone does not — but the scan range degrades to the origin default branch with a loud stderr `WARNING` naming the unresolved base; it NEVER degrades to the branch root commit, which would be strictly wider than the pre-#439 range. `/solve` and `/turbo` do not read this key — their Workflow fleet inherits the launcher's current branch automatically via the `baseBranch` envelope scalar.
...
**`bot_authors_allow_list` semantics:** **DEPRECATED.** As of the unconditional-autopilot release, `/merge` does NOT gate on PR-author identity — every APPROVED + CI-green PR is eligible regardless of whether the author is a collaborator, a bot, or an external contributor. Phase 1.4 trust resolution accepts EITHER `reviewDecision == "APPROVED"` (PATH_1, team / branch-protection path; required reviews + status checks anchor the trust) OR an uberdev trust trail bound to the PR's trust head, resolved by the `trust-trail-evaluator` agent (PATH_2, solo-dev / no-protection path). PATH_2 is generalised over the trail's **instrument** — the command that produced it, carried as DATA inside the trailer (`Reviewed-by: uberdev/<instrument>@<sha>`) rather than baked into a fixed prefix — so it accepts a `/review-pr` trail (label `uberdev-approved` or `uberdev-approved-with-concerns`) and a `/premerge` stack trail (label `premerge-approved`) on the same terms. The label and the trailer must name the SAME instrument, and a `premerge` trailer must additionally carry a `gate=green attempt=NN` token in the immutable commit body, so that tier never rests on a bare label plus a bare trailer. The trust head is the PR's head SHA, except that one provably inert `chore(release): vX.Y.Z` commit on top is tolerated and read through (#364). The agent reads structural primitives — ancestor + diff-empty + log-empty — plus the caller-computed base-identity inputs that pin the reviewed delta's other endpoint (#440), and returns verdicts in `{PASS, STALE, INVALID, FORCE_PUSHED}`; a PASS lands `trust_anchor="uberdev_review_trail"` or `"uberdev_premerge_trail"` so an audit consumer can tell the two tiers apart. Author identity is NOT a gate in either path; the new trust trail does not re-introduce author-identity gating. The key is parsed without error for backward compat but has no behavioural effect.
