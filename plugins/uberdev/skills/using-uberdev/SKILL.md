...
Some skills mandate a `Workflow` tool call as their execution path: the preflight prints args JSON between `WORKFLOW_ARGS_BEGIN`/`WORKFLOW_ARGS_END` markers and names a `workflow.js` under `${CLAUDE_PLUGIN_ROOT}`. That call is sanctioned — it is the documented opt-in path; invoke it with that scriptPath, relaying the emitted args verbatim. If `Workflow` is not among your tools, follow the skill's `## No-Workflow fallback` section instead.
