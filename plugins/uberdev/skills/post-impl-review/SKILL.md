...
The seven lenses, their agent files, and the `POST_IMPL_REVIEW_CAP` precedence
chain (`fanout_cap` input > env > config > default 7) are in
`references/contracts.md`. Read it before dispatching: the cap decides whether
the seven form one wave or several, and every wave still obeys
...
The evidence ledger is independently roster-bound — stable edge, original
`REVIEW_EDGES` index, launched instance, child-owned `validated-result.md` path,
validated digest. `references/contracts.md` carries what the final gate requires
and every way it fails closed.
...
The remaining evidence-failure classes — `result-rewritten-after-status` and
what separates `incomplete-roster` from a truncated ledger — are in
`references/contracts.md`, with the corroborating wait-boundary lines each one
needs.
...
ABOUT A DOCUMENT, so it is admissible only with its own deterministic filter:
every quote is verified verbatim against the cited file's own bytes, and a
finding whose quote cannot be located is **culled**, not downweighted. The
writer takes three more REQUIRED inputs for it and adds four fail-closed
classes. The whole gate — the five cull rules, the demotion for a rule this PR
itself wrote, and the `convention-citations.md` log that is published even when
nothing was culled — is in `references/contracts.md`.
...
## Failure modes and integration

Reviewer supervision blocked, or a terminal reviewer still malformed after its
one `attempt02` repair, both suppress the ordinary aggregate, return blocked,
and prevent fixer/trust dispatch. The full table, the only live caller, the
findings-artifact contract (writer, path, reader, read shape, failure
boundary), the `finish-branch --interactive` pre-push opt-out, and the
anti-loop "does NOT call" list are in `references/contracts.md`.
