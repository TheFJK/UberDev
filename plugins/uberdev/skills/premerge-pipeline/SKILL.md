...
`/premerge` packs unconditionally, accepts a stack of one, and has **no invoking
PR** — so `review_consolidate_assert_current` has no referent and is deliberately
NOT called. In its place Phase 0c asserts the strictly stronger thing: every
discovered candidate ends the phase either **included** or **excluded with a
typed reason**, and a candidate in `candidates.json` that is in neither ledger
halts the run. The full comparison table is in `references/phase-contracts.md`.
...
### What a failed push is allowed to say, and what the stack PR carries
...
### What `PREMERGE_PUSHED` is at each call site, and the CI settle window

The gate takes `PREMERGE_PUSHED` with `:?` and no default, so the value at every
call site is part of the contract rather than a convention.
`references/phase-contracts.md` states it per site, and carries the settle-window
rule that `PREMERGE_WAIT_CI_CEILING` bounds — a check that never settles ends the
loop as *not-green*, never as a pass.
...
`MAX_FINDINGS = 64` bounds one aggregate, and a long stack can exceed it
honestly. Which sentence the run prints is decided by `TOTAL=`, `BLOCKER=`,
`SUGGESTION=` and `FILES=` together — never by `OVERFLOW=` alone, because rows
are also dropped for having no usable `file`. The full arm-by-arm contract, the
...
Every mistake this pipeline has shipped, or been one edit away from shipping, is
listed in `references/common-mistakes.md` — merging, skipping the gate before the
fixers, treating a `PLAUSIBLE` verdict as a lower severity, bumping by hand, and
removing the accounted-for gate in 0c. Read the entry a refusal names before
working around that refusal.
