...
**A shard must run its whole plan, and the harness now says so out loud.** Every sharded `run:` block closes with a `SHARD-COVERAGE` tail, printed just ahead of the failure summary: it names that shard's plan length (`plan`), the number of invocations that went through the shard filter at all (`routed`, so the job's whole fixture list, not the shard's share of it), and the number the filter let through (`ran`), and it raises the block's failure count whenever `ran` and `plan` differ — or whenever the plan is set but its length reads as zero or non-numeric, which would otherwise let the comparison pass by erroring out. Wiring alone cannot promise this. The workflow can name every fixture, `tests/ci-shard-plan.py` can partition them correctly, and a shard can still execute one of them, because the filter's answer to a name it cannot find in the plan is `--- SKIP`, and a skip is not a failure: `shape-checks-windows` ran 6 of its 70 fixtures across six shards and reported green (#753). The tail measures the plan with `wc -w`, which splits on any whitespace, while the filter matches on single spaces — that asymmetry is deliberate, because a guard that re-derived the length the way the filter derives it would agree with a corrupted plan by construction and see nothing. `tests/ci-shard.test.sh` row S10 asserts the same property from outside CI, replaying each job's real `run_one` argv against its real per-shard plan and comparing the **aggregate** run count against the job's wired fixture count.

**Reading the plan bytes.** The plan crosses a `$GITHUB_ENV` file between the planning step and the test step — one step writes it, the runner reads it back and re-exports it — and a delimiter defect at that boundary leaves no trace in a normal log. So every sharded job now runs `bash tests/ci-shard-diagnose.sh dump <job-key> <shards> <shard>` between the two. It is a **reporter, never a gate**: it always exits 0, and the gates are the coverage tail above and the CR refusal in `tests/ci-shard-env.sh`. Section **[A]** re-runs the producer and `od -c`s the plan line it emits, **[B]** is an `od -c` of `UBERDEV_SHARD_PLAN` as the dumping step inherited it through the file, and **[C]** probes every planned name through the filter's own membership rule and prints a `match=` / `miss=` tally. Either side alone would be ambiguous — a plan carrying a stray delimiter and a clean one can look identical from one end — which is why both are dumped. The same script carries three more verbs for local use, and its `TWO SHIMS, NOT ONE` header is load-bearing about why they are not interchangeable. `packer-bytes` is the one that drives the real packer with its stdout wrapped in a CRLF-translating text stream — in-process, which is what `python3` **is** under Git Bash — and the packer's own `sys.stdout.reconfigure(newline="\n")` is expected to override a wrapper installed around it from outside: against the shipped pin the verb comes back LF-only, and that flip is the assertion. `simulate` must **not** inherit that flip, because it drives the *consumer* — the CR refusal in `tests/ci-shard-env.sh` — and a guard whose only input arrives through the producer it is meant to test stops being tested the moment that producer is pinned. So `simulate` runs the packer unwrapped and re-applies the CR at the **process boundary**, downstream of every byte the packer has already written, where an in-process `sys.stdout.reconfigure` cannot reach it: the same pinned packer that yields `packer-bytes` zero CR still trips the refusal here (exit 2). `crlf-plan` involves no CRLF stream at all — it captures the real per-shard plan and applies the delimiter model to it directly, CR before every separator and none after the last entry.

**What is settled, and what is still a model.** The *consuming* half is settled by execution, on any host: drive the live harness with a plan whose entries are CR-terminated and whose final CR was consumed at the file boundary, and exactly one name is left bounded by spaces on both sides, so exactly one fixture runs — the reported signature. Give every entry a trailing CR, the last one included, and nothing runs at all, which is not what CI reported, so that shape is refuted. The *producing* half is **not** settled. The working model is that under Git Bash `python3` writes stdout as a text stream and terminates each line CRLF; against that, `tests/ci-shard-plan.py` pins its own stdout to LF, and `tests/ci-shard-env.sh` refuses a CR-bearing pack outright (exit 2) rather than stripping it, so a regression in the pin kills the planning step instead of silently shrinking the suite. What can be shown off Windows is only that the pin is **sufficient** against interpreter-level newline translation — not that such translation is the cause on `windows-latest`. Settling that needs a Windows runner, and the dump step is where to read it: open the first `shape-checks-windows` run after this landed, take [A], [B] and the [C] tally, and state the delimiter actually observed. A plan of length N reporting `match=1` there, or a coverage failure on a job whose packer is already pinned, **refutes the model** — the cause would then lie elsewhere on the path, and this fix should not be treated as the fix.
...
- The root `CLAUDE.md` — the bump-version-everywhere ritual the version-lock tests enforce, and which commit in each lane carries the bump.
