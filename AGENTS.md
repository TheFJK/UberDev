...
|---|---|---|
| **`/goal`** | a `chore(release): vX.Y.Z` commit the pipeline pushes onto the PR branch, immediately before it dispatches `/merge` | `_uberdev_goal_ensure_version_bump` (`plugins/uberdev/lib/goal-state.sh`), called from the barrier-gated merge dispatch in `plugins/uberdev/lib/goal-watch.sh` (step 2c). It **fails closed**: any status it does not recognise as "bumped" withholds the `/merge` dispatch and writes a `goal_merge_deferred` audit row with `reason=version_bump_failed`. Landing unbumped is the bug, so merging is never the fallback. |
