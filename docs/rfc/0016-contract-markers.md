...
| `goal-circuit-breaker-reason` | — | 10 | 2 | **not in the register** — found by applying this convention; the run-state allowlist carries `-solver_failed` |
| `sdd-implementer-status` | — | 5 | 4 | **not in the register** (#517) — the SDD implementer's terminal status, declared at the child-facing output contract, the role card, the controller's branch block and the prompt template. Set equality here cannot see a member with **no handler**, nor the prompt-assembly defect that was #517's root cause — a contract-less edge whose assembled prompt ended with a `lib/child-dispatch.sh` fallback directive that NAMED a vocabulary of its own, last and therefore authoritative. #546 retired that wording: the contract-less arm now points at the return contract the role card declares, which removes the override on all 32 unbound provider edges (31 of them ever composed — `solve.issue.lead` has `role: null`) but replaces a contract with a **pointer** — 2 of the 20 unbound roles (`code-reviewer`, `pr-test-analyzer`) declare their return shape as a prose header rather than a fenced block, so for them it resolves weakly (#580). The classifier is stated rather than left to the reader, because leaving it implicit is how this row came to name a third role (#606): a role resolves strongly when it carries a fenced block in its role card, whatever heading that block sits under — a heading-shaped reading of the same question answers differently, since one card declares its fenced schema under `## Review` rather than under `## Return contract`. `tests/solve-run-tree.test.sh` (`SRT-606.0`–`SRT-606.2`) derives both that subset and this count from the cards on disk; binding those edges to real output contracts is #578, and the three register families #517 deferred — including the uberthink one whose registration must delete both `TWIN_ALLOWLIST` entries under this contract — are #579. Both gaps are held on the bytes by `tests/sdd-child-inputs.test.sh` (`AC-14`, `AC-14a`, `AC-14b`) |

The last row is the point of the exercise: the mechanism found a tenth member of
the family on its first application. `GOAL_CIRCUIT_BREAKER_REASONS` declares ten
halt reasons — nine until issue #624 added `backend_resolve_failed`, which joined
both this enum and the allowlist below — while the `CIRCUIT_BREAKER_HALT`
allowlist inside `uberdev_goal_read_run_state` enumerates nine. It is latent
rather than live — today only `agent_stuck_on_dialog` is ever assigned to that
scalar — but the arm has no else branch, so the residue shape is identical to
the one that broke `UBERDEV_RESOLVED_BACKEND` in #360. The divergence is
declared at the site with a comment saying it is drift, not intent, and it now
reds on any one-sided edit.
