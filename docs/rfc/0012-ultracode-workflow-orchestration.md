...
2. **zsh/bash divergence.** The Bash tool runs `/bin/zsh` on macOS:
   - `ARR=($VAR)` scalar-splits both scan wave loops into one garbage element (`CHUNK_ARRAY=($CHUNK_IDS)` in the uberscan and ubersimplify wave loops, #305);
...
3. **Skill-renderer `$ARGUMENTS` mangling.** Positional args substitute into the *whole* SKILL.md body:
   ...
   - `/testers http://localhost:3000` corrupts `_wave_count`'s `"$1" "$2"` (testers-pipeline).
...
**`skills/goal-pipeline/workflow.js` (goal-R3, as shipped)** — the driver script named in the §3.0 table. Args come from `lib/goal-phase0.sh` (the args envelope, verbatim-relayed per DR-2): goal id, tmpdir, plugin root, bash path, queue, `maxCycles`, `maxParallel`, barrier timeout, review grace, watch budget (contract-asserted ≤ 540 s), stuck window, backend, `onlyMine`, start epoch.
