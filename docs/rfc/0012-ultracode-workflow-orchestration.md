...
| **Target ver** | staged 0.37.0 → 0.4x per §9 roadmap (current `main` at authoring time was 0.36.1; every release bumps the two `assert_version_bump` locks — the `G20: version bump locked` block in `tests/goal.test.sh` and the `Version bump A.B.C -> X.Y.Z propagated` block in `tests/solve-claim.test.sh`) |
...
> **Reference convention (normative for this RFC).** Live, consumer-facing claims cite code by **symbol** — function name, constant, enum member, or a named `Step`/`Phase`/section heading — never by `file:line`. Line literals rot the moment the cited file is edited, and the edit that moves the code is usually the same commit that would have updated the prose, so the rot is silent by construction: by the time §3.3's contract table was first audited, one "audit-JSON write" anchor pointed at sequential fanout-cap prose, one marker anchor landed inside a Python heredoc, and the trust-reader anchor landed on the audit *emitter*. This section and the §3.3 contract table — the two surfaces consumers act on — are symbol-only, and `tests/docs-accuracy.test.sh` locks that. Any `file:line` still visible in §3's historical narrative below is a **snapshot of the pre-migration tree at authoring time**, kept for provenance; treat it as a description, never as a pointer to resolve.
...
- **jq / `|| echo 0` masking.** Crashed producers read as count=0 (memory: `project_uberdev_jq_echo0_masks_crash`). Maximum blast radius: uberthink runs its Wave-4/7 python under `2>/dev/null || true`, so an ImportError becomes CB-CONVERGE and the user is told their goal "admitted no feasible novel approach" after a ~90-minute run (#307).
...
5. **Workflow agents are Task-style subagents, NOT detached full Claude sessions**: they cannot replace claude-bg dispatch semantics (independent permission tier, survive-the-parent, hours-long lifetimes). A child `workflow()` per issue CAN replace an in-session SDD fanout — never a detached background solver session.
   > **SUPERSEDED IN PART — RFC 0015 (`auto` no longer resolves to a detached backend; shipped for `/solve` + `/turbo`, staged for `/goal`).** RFC 0015 §2 **narrows** this constraint rather than overturning it, and the three properties above stay the honest cost list: *independent permission tier* is re-read as an anti-feature for work the user just asked for (`/turbo`'s unattended mode is `autoMode` in args, not a second permission domain); *survive-the-parent* is a real, accepted loss (RFC 0015 §6 R-1); *hours-long lifetimes* survive because the runtime, not the script, owns agent lifetime. What remains binding: a Workflow agent is still not a detached session, so anything that must genuinely outlive its session — `/goal`'s cross-session durability above all — still needs the on-disk store, and the constraint below on nesting still caps how much of a run one `workflow()` level can carry.
...
1. Preflight: flags, single PR bind, then atomic RUN_ID directory reservation + regex validation. An explicit collision fails; an internal mint retries bounded cryptographic discriminators around one timestamp/HEAD prefix. Write the locked marker in that reserved directory before any burst and export one opaque identity/digest reservation receipt. The directive fallback installs no `EXIT` trap: explicit setup failures abandon through the receipt, while normal interruption leaves freshness-bounded in-flight evidence.
2. Workflow call.
...
4. Compute final GREEN/YELLOW/RED incl. OVERRIDE_GREEN; **emit trust artifacts in the load-bearing order: anchor commit → push → labels → verdict JSON LAST with post-push `ANCHOR_SHA`** (the /goal SHA-binding contract — §3.3 table). A self-contained fresh-shell fence rehydrates the setup receipt, validates both markers, and uses the shared exact-name no-clobber publisher. Failed publication never removes its pathname; only successful publication retires the markers. RED still writes the JSON (review-pr.md:807); skipping it strands /goal in the stale|missing cascade. `review-pr:pending` cleared on green only.
5. Append `auditEvents[]` to `.uberdev/audit.jsonl` JSON-encoded (C-5); exit 0/1/2 — the contract `/merge` `Step 1.4.5 — Auto-review intercept` reads synchronously and finish-branch chains on. **Marker retirement belongs to step 4, not to this step:** only successful verdict publication retires the two receipt-bound markers, so no unconditional marker removal happens here. An unconditional removal would erase the in-flight evidence a failed publication is supposed to leave behind, and re-open the stale-marker window `_uberdev_goal_locked_marker_for_pr_fresh` exists to bound.
...
### 3.3 `/goal` — hybrid driver; nested solve-fleet workers, serialized merges, disk store untouched

> **SUPERSEDED IN PART — RFC 0015.** This section was written when `auto` resolved to a detached `claude-bg` session and constraint 5 therefore forbade moving the workers. It no longer does: `/goal`'s solvers ride the same `workflow` backend as `/solve`, dispatched as ONE nested `workflow()` call per cycle into `skills/solve-fleet/workflow.js`. What survives unchanged is the *reason* the driver/worker split was drawn here — the watch lane cannot sleep, poll a timer or wait on CI inside a script, and resume is same-session only, so cross-session durability still rides `lib/goal-state.sh` on disk (RFC 0015 §5). The claude-bg cost list below is retained as the honest accounting of what the detached transport bought and what its retirement gives up.

`/goal` is the one pipeline whose bash actually *runs* (a real 60 s watch loop with live breakers) and which fires **zero** Task calls — pre-RFC-0015, every worker was a detached session via `uberdev_dispatch_one`. The migration replaces the **driver** first: before it, the main session was the bounded-tick harness described in `commands/goal.md`, paying a full-context turn per tick (up to ~27/cycle at WATCH_BUDGET=540, × 5 cycles) plus ~18 fence-boundary turns, all riding a conversation seeded with 127,042 B.
...
**`skills/goal-pipeline/workflow.js` (goal-R3 — designed below, NOT yet on disk; §9 sequences it in Phase 7)** — the driver script named in the §3.0 table. Args come from `lib/goal-phase0.sh` (the args envelope, verbatim-relayed per DR-2): goal id, tmpdir, plugin root, bash path, queue, `maxCycles`, `maxParallel`, barrier timeout, review grace, watch budget (contract-asserted ≤ 540 s), stuck window, backend, `onlyMine`, start epoch.

```text
let queue = [...args.queue], cycle = 1                 // queue/candidates/fingerprints are JS
try {                                                  // variables spanning the WHOLE run — the
  while (true) {                                       // fence-death false-converge (#301) and the
    if (cycle > args.maxCycles) return finalize('max_cycles')   // rollover-wipe class are gone by
    if (budget.total && budget.remaining() < RESERVE) return finalize('budget_halt')  // construction
        // RESERVE sized above one full cycle's spend
    phase('dispatch'):
        (a) ONE haiku relay agent runs lib/goal-phase1.sh — CLAIM ONLY (label provisioning, the
            uberdev:active SETNX, MAX_PARALLEL rollover, input→dispatched); it dispatches nothing
            and prints one compact JSON line
            schema {rc, dispatched[], rolledOver[], claimSkipped[]} ; rc!=0 → finalize('dispatch_failed')
            log(claimSkipped + rolledOver EVERY cycle — cross-process claim collisions surfaced only
            on fence stderr before, which workflow transcripts never show)
        (b) EXACTLY ONE nested workflow({scriptPath: skills/solve-fleet/workflow.js}, <cycle args>)
            for the WHOLE cycle, try/caught, behind a projected-agent gate. NOT one call per issue:
            solve-fleet already does the per-issue parallel() fan-out itself, and per-issue nesting
            would spend the single nesting level on the wrong thing, leaving the fleet's own agents
            unable to run. Read {results[], counts, prsOpened, cb1Tripped, cb2Tripped} from the return.
            goal-phase1.sh invokes lib/solve-launcher.sh --backend=workflow to produce the fleet
            manifest + args envelope and hands the JSON between WORKFLOW_ARGS_BEGIN/END to the nested
            call VERBATIM — the driver never hand-composes that envelope.
    phase('watch'): bounded for-loop of haiku tick agents whose Bash runs lib/goal-watch.sh
        (merge lane + wall-clock breakers only) with the documented 0/42/1 exit contract:
        schema {exitCode, breaker|null, census{solving,reviewing,merging,held,merged}} ; log(census)
        exit 0 → drained; exit 1 → finalize(breaker); null tick → one retry then finalize('tick_lost')
        // the script has NO timers, only a loop counter (constraint 3): the 60 s cadence lives INSIDE
        // the tick agent's Bash, and wall-clock logic (4 h stuck, 60 m grace, solve timeout) stays
        // bash-side against persisted epoch anchors (DR-7)
        // trust verdicts are NEVER LLM-reported: a relay is handed the verdict PATH and runs
        // uberdev_goal_read_trust_signal
    if (!drained) return finalize('stuck_loop')
    phase('collect'): ONE agent runs lib/goal-phase3.sh (candidates query → fingerprint repeat →
        overflow truncation → terminal gates, IN THAT ORDER)
        schema {rc, verdict ∈ [converge, halt, loop], reason|null, candidates[]}
        converge → finalize('converged'); halt → finalize(reason)
    queue = queue.concat(c.candidates)   // rollover merges, never overwrites; bash still mirrors to
    cycle++                              // disk for crash forensics
  }
...
**Nesting-level accounting (changed by this design — read with §3.6).** `/goal` **spends the single `workflow()` nesting level** on its per-cycle solve-fleet call. Everything the fleet needs must therefore run as a leaf inside it: the SDD wave logic §3.6 reserved that level for **cannot** be a child workflow of a /goal-driven solve child and is inlined into the solve child's own orchestration instead. A second `workflow()` inside the fleet script throws at runtime, so this is a hard structural constraint, not a preference — and it is worth asserting directly (the fleet script must contain zero `workflow(` call sites).

Revisions baked in (from falsification): tick, dispatch AND collect prompts all mandate Bash `timeout: 600000` (≥ WATCH_BUDGET + one pass of gh latency — the Bash-tool default is 120 s, and an unpinned prompt gets its watch fence SIGTERMed mid-pass); an **operator abort path ships with the driver, not later** — `lib/goal-abort.sh` reads `goal-active-id.txt`, releases every non-terminal `uberdev:active` claim and clears run state. With detached sessions retired this is load-bearing rather than a convenience: workflow children die with the session, so a closed window would otherwise strand `uberdev:active` labels forever (workflow-cancel cleanup semantics remain unverified — R-2). The SKILL.md feature-detects the Workflow tool and falls back to the tested bounded-tick main-session harness, which goal.md keeps documented as the PERMANENT non-Workflow drive — using-uberdev targets Copilot/Gemini CLIs that have no Workflow tool — not a stopgap (the W2a-W2f contract stays the canonical tick API consumed by both drivers); the run is **session-bound** — background ≠ survive-the-parent, and post-RFC-0015 that now costs the solvers too, not just the driver (RFC 0015 §6 R-1); `resumeFromRunId` is **forbidden** for the goal driver (DR-9 — prefix replay would return cached "dispatched/merged" results against a moved world); the preflight verifies the orchestrating session itself runs with skip-permissions/allowlist coverage (background agents cannot answer prompts — constraint 1). The `UBERDEV_TURBO=1` / `SKIP_PERMISSIONS=1` / `UBERDEV_GOAL_ID` / `UBERDEV_RESOLVED_BACKEND` env injection stays the contract for any remaining explicitly-selected detached backend — without it review-pr's 6c.6 non-turbo arm AskUserQuestions inside a headless session (gate 1 is TTY-safe by design; gate 2 branches on TURBO only).

**Keep (goal-R4):** the lowest-first + MERGING-interlock merge barrier (parallelizing it re-opens the version-bump-collision class that bit twice); the goal-state store layout byte-stable (goal-`<id>` TSVs/jsonl + runstate sidecar + `goal-active-id.txt`) — #310's reader and /merge PATH_2's trust trail depend on it; `--backend=wezterm` and `--backend=background` as explicitly-selected transports, which is why the sidecar backend allowlist, the reaper-skip lane and the busy/in-flight probes must carry a `workflow` arm ALONGSIDE them rather than replacing them. (AMENDED 2026-08-05, issue #381: `claude-bg` was DELETED, not merely deprecated — the enum is now `auto|workflow|wezterm|background`. See RFC 0015 §1 and §7.) Per-pass gh-memoization speedups (#301c) live in bash helpers agents still execute — do independently, any time.
...
**Joint-migration contract table.** /goal consumes only file globs, gh state/labels, and dispatch-receipt metadata — which is exactly why the three migrations are feasible, provided these literals survive byte-identically (acceptance checklist for Phases 4/5/7).

**Anchoring rule for this table (normative).** Every row cites its producer and consumer by **symbol** — a shell function, a constant, or a named `Step`/section heading — and **never** by `file:line`. This is not cosmetic: the previous line-literal edition rotted silently within a release (its "audit-JSON write" anchor came to point at sequential fanout-cap prose, its locked-marker anchor at a Python heredoc, and its trust-reader anchor at `uberdev_goal_audit` — the *emitter*, not the reader), because the commit that moves the code is the commit that would have fixed the prose. Symbols move with `grep`; line numbers do not.

| Contract | Producer → Consumer | Must preserve |
|---|---|---|
| Verdict JSON | `/review-pr` `## Trust-Signal Emission` → canonical selector `discover_review_verdict_json` (`skills/merge-pipeline/lib/discover.sh`) → `uberdev_goal_locate_review_pr_audit_by_pr` | `<checkout-toplevel>/.uberdev/runs/<RUN_ID>/review-pr-verdict.json` inside the PR checkout (root or the three worktree layouts); strict positive-integer `.pr` and lowercase 40-hex `.sha`; rank only the 15-byte timestamp prefix; selected-timestamp expected-PR ties require byte-identical captured payloads. Known other PRs do not rank; unknown identity **at or after** the selected timestamp is indeterminate (a strictly older unknown is ignored). Root symlinks may target external storage but remain identity-bound. Consumers receive a closed snapshot receipt, normalize it once, clean it, and never reopen mutable paths. |
| Trust fields | `/review-pr` `## Trust-Signal Emission` → canonical selector → `uberdev_goal_read_trust_signal` | `.sha`, `.phases.phase2_5.by_severity.{blocker,critical}`, `.halted`, `.halted_due_to_overflow`; missing/null `phases` or missing `phase2_5` is legacy/stale, while a present Phase 2.5 object is strict-typed. Blocker and critical acceptance are independent even when `halted=true`: each nonzero tier requires its corresponding explicit flag; neither flag implies the other. |
| SHA binding | `/review-pr` trust-artifact emission (`## Trust-Signal Emission`) → `uberdev_goal_read_trust_signal` | emission order anchor-commit → push → labels → **JSON last** with post-push ANCHOR_SHA, within `_UBERDEV_GOAL_DEFAULT_REVIEW_GRACE_SECS` — JSON-before-anchor makes every verdict permanently `stale` |
| RED writes JSON | `/review-pr` RED arm of `## Trust-Signal Emission` → the /goal red-held path | the RED path still emits the verdict JSON (else stale\|missing cascade → dispatch-cap exhaustion → `queue_empty_not_converged` instead of clean red-held) |
| Locked marker | `review_reserve_run_directory` in `/review-pr`'s `### Executable setup` fence → `_uberdev_goal_locked_marker_for_pr_fresh` (grace from `_UBERDEV_GOAL_DEFAULT_REVIEW_GRACE_SECS`) | atomically reserve `runs/<RUN_ID>` with plain `mkdir`, then write `{locked, pr-context.json{pr:int}}` BEFORE the fanout in the checkout toplevel; explicit collisions fail and internal IDs use bounded cryptographic-discriminator retry. Setup emits one closed identity/digest receipt and installs no `EXIT` trap. Explicit setup failures abandon through that receipt; finalization revalidates it, publishes the verdict exact-name/no-clobber, then removes only the two markers. mtime = THIS run's write time and stale interruption remains freshness-bounded. |
| Finding issues | `findings-to-issues` → the /goal Phase-3 candidate scan + the held-PR unblock check (`_uberdev_goal_parse_blocks_line`, `_uberdev_goal_extract_fingerprint`) | label `review-pr-finding`, `**Tier:** BLOCKER\|CRITICAL` bold-markdown, 16-hex fingerprint HTML comment (`FINDING_FINGERPRINT_REGEX`), `Blocks: #<PR>` backref; same `gh` identity for `--only-mine` |
| GREEN label | `/review-pr` label emission → `uberdev_goal_batch_unblock_wait_clear` | literal `uberdev-approved` (the phantom `review-pr:green` deadlocked the barrier pre-#289.1); `review-pr:pending` cleared on green ONLY (it is /merge's auto-review backstop trigger) |
| In-flight probe | `_uberdev_goal_dispatch_review_pr` → `uberdev_goal_review_pr_in_flight` | the child prompt contains the literal `/uberdev:review-pr <N>` followed by a non-digit — the probe's regex matches on that shape, so any rewording of the imperative silently blinds it |
| Dispatch plumbing | `uberdev_dispatch_one` (`lib/dispatch.sh`) → `_uberdev_goal_pid_for_issue`, `uberdev_goal_agent_busy_for_issue`, `_uberdev_goal_any_attempt_stuck`, `_uberdev_goal_reap_zombies` | `solve-bg-status-<key>.json` (`.pid`), `solve-bg-stdout-<key>.log` (mtime activity proxy), worktree/cwd `solve-issue-<key>` naming — else the stuck-breaker and reaper go blind and live merges look idle at 60 m. **Post-RFC-0015:** these receipts are produced only by the explicitly-selected detached backends; the `workflow` backend has no PID to supervise, so every consumer above needs an explicit `workflow` arm rather than falling through the detached-session catch-all. |
| Merge result | `/merge` `Step 3.2 — Clean-merge path` / `Step 3.3 — Conflict-resolve path` audit rows → `uberdev_goal_read_merge_result` | `merge_executed`/`pr_parked` rows appended to **top-level** `.uberdev/audit.jsonl` (NOT `runs/<id>/` — the documented-but-unwritten path), int `data.pr`; park enum `refused\|ambiguous\|test-fail-exhausted\|push-non-ff`; gh state MERGED via real `gh pr merge`; CLOSED-without-merge = immediate `merge_failed` halt |
| Claim release | `/merge` `Step 3.4 — Post-merge issue cleanup` → the /goal claim protocol (`uberdev_goal_issue_state_transition` + the `uberdev:active` SETNX in the claim phase) | Step-3.4 `uberdev:active` label/assignee removal survives the migration — it is the ONLY releaser on the merge path; without it every merged issue soft-skips (rc 2) forever |
| Merge invocation | `_uberdev_goal_dispatch_merge` → `/merge` | bare `Invoke the slash command /uberdev:merge <pr> now.`; `UBERDEV_GOAL_ID` rehydrated before `uberdev_goal_should_automerge` (provenance gate + attempt cap ×3) |
| Partial-chain flag | `finalize()` in `skills/solve-fleet/workflow.js` (per-issue `chainComplete` → the `prsPartial` subset of `prsOpened`) → the partial ledger in `skills/goal-pipeline/workflow.js` → the `merge-dispatch-gate` region of `lib/goal-watch.sh` and the `terminal` region of `lib/goal-phase3.sh` | the carriers are command-line flags — `--partial-prs=<csv>` on `lib/goal-watch.sh`, `--partial-issues=<csv>` on `lib/goal-phase3.sh` — because the driver may not touch the filesystem (§2.2 constraint 6) and the merge gate never sees a JS value; digits-and-commas or a startup exit 2 at each script's own edge; **omitted, never emitted empty** (an empty value is that same refusal, so an always-on flag halts every converging run); `chainComplete` read strictly as `=== false` (absent means no task chain ran at all); the PR half stays a subset of `prsOpened` by taking the fleet's own `prsPartial` rather than re-deriving it, so the claim-collision rule has exactly one implementation |
| Exit codes | `/review-pr` `## Exit-Code Contract` → `/merge` `Step 1.4.5 — Auto-review intercept` | 0/1/2 surfaced as the Skill() return for /merge's synchronous Step-1.4.5 and for finish-branch; /goal never reads rcs (the contrary claim in `commands/goal.md` is stale prose) |

### 3.4 `/solve` + `/turbo` launcher — hybrid: shell preflight (`lib/solve-launcher.sh`) + `skills/solve-fleet/workflow.js`

> **SUPERSEDED IN PART — RFC 0015.** The "keep shell" verdict rested on one premise: the launcher's heavy step is *detached* dispatch, which constraint 5 says a Workflow agent cannot be. RFC 0015 rejects the premise rather than the reasoning — `/solve` never needed a detached session — so `auto` now resolves to the `workflow` backend and the per-issue solvers run as worktree-isolated Workflow agents under `skills/solve-fleet/workflow.js`. The extraction below is **still exactly right and shipped**: it is now the preflight half of the hybrid, with a `Step 5w` that writes the fleet manifest, emits the args envelope and exits WITHOUT dispatching (ordering against `Step 5b'` is test-locked — a run that did both would double-dispatch against a single set of claims). ~~`--backend=claude-bg` / `--backend=wezterm` remain available and deprecated (removal target v1.0.0)~~ — **AMENDED 2026-08-05 (#381): `claude-bg` and `codex` were both deleted, not deprecated; `wezterm` and `background` are the two detached transports that remain** — which is why the "keep as-is" bullet below still describes live code.

The launcher has zero Task fanout; pre-RFC-0015 its heavy step was **detached** dispatch (constraint 5) — a Workflow was judged the wrong tool for it (LLM agents running 3 gh calls each is waste). That judgment stands for the *preflight* work, which is why it stays shell: extract Phase A + Step 4.5 + Phase B into an executable `lib/solve-launcher.sh` run as ONE Bash call.
...
- `sdd-waves.js` **never calls `workflow()`**. The original reason was to reserve the single nesting level so solve-design.js could later invoke SDD as a child workflow; **§3.3 spends that level instead** — under `/goal` the driver's per-cycle `workflow()` into `skills/solve-fleet/workflow.js` IS the one permitted nesting, so the fleet's solve child must inline its wave logic rather than nest a second script. The rule survives its rationale, and hardens: a `workflow()` here would throw at runtime on the /goal path.
...
- **/issue keeps** (light-R1): Phase 5 is a deliberate draft-confirm gate protecting a GitHub mutation (issue.md:178-180) — constraint 1; the entire fanout is ONE wave of TWO read-only scouts consumed inline on a <30 s-median command (issue.md:13); zero loops/breakers/cross-fence state. Edit (light-R7): stop splicing raw `$ARGUMENTS` into the double-quoted `echo` fence (issue.md:24-26 — `$(...)`/backticks in a description are injection-shaped); adopt dev-pipeline's model-side token parse, keeping only `REPO=$(gh repo view ...)` in bash. Keep the test-locked deprecation literals (issue-causal-fanout.test.sh:103-108). **SUPERSEDED IN PART (#723).** The light-R1 *keep* verdict still stands and so do both reliability edits: `/issue` is still ONE wave of TWO read-only scouts consumed inline on a <30 s-median command, with zero loops, breakers or cross-fence state, and the model-side token parse shipped. What no longer holds is the premise inside constraint 1 — Phase 5 is no longer a draft-confirm gate. It is now an open-duplicate gate: the GitHub mutation is still protected, but by the one fact that makes creating wrong (`triage-scout` reporting a live duplicate) rather than by a blanket approval prompt. Same reasoning that made `auto_confirm` a no-op for `/merge`, and the same policy `README.md`'s "No HARD-GATE approval checkpoints" row states. The sentence above is retained verbatim as the record of what was decided here, not rewritten.
...
| Issue | Disposition |
|---|---|
| #301 goal | **do-first** correctness trio (goal-R1) + the 117 KB diet executed as migration prep (goal-R2 four-script extraction); watch-driver items then **superseded** by `skills/goal-pipeline/workflow.js`; per-pass gh-memoization speedups **unaffected** (live in bash helpers agents still execute) |
...
2. Cancel/cleanup semantics (R-2) — empirical; gates the goal workflow driver becoming the default.
