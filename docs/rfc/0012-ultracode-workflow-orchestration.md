...
1. **Fence-scoped shell state death.** Each bash fence is a fresh process; `trap EXIT`, PID stamps, exports and arrays die at fence end:
   ...
   - historically, `/review-pr`'s locked marker + EXIT trap lived milliseconds (review-pr.md:88-106, #302); the directive fallback now carries marker authority across shells in an identity/digest receipt, while the `EXPECTED_OLD_SHA` force-push lease still splits across fences (review-pr.md:554→621);
...
|---|---|---|
| Verdict JSON | review-pr.md:954 → canonical selector → goal-state.sh:791-811 | `<checkout-toplevel>/.uberdev/runs/<RUN_ID>/review-pr-verdict.json` inside the PR checkout (root or the three worktree layouts); strict positive-integer `.pr` and lowercase 40-hex `.sha`; rank only the 15-byte timestamp prefix; selected-timestamp expected-PR ties require byte-identical captured payloads. Known other PRs do not rank; newer/equal unknown identity is indeterminate. Root symlinks may target external storage but remain identity-bound. Consumers receive a closed snapshot receipt, normalize it once, clean it, and never reopen mutable paths. |
| Trust fields | RP:956-990 → canonical selector → GS:633-713 | `.sha`, `.phases.phase2_5.by_severity.{blocker,critical}`, `.halted`, `.halted_due_to_overflow`; missing/null `phases` or missing `phase2_5` is legacy/stale, while a present Phase 2.5 object is strict-typed. Blocker and critical acceptance are independent even when `halted=true`: each nonzero tier requires its corresponding explicit flag; neither flag implies the other. |
...
| Locked marker | RP:85-109 → GS:1354-1380 | atomically reserve `runs/<RUN_ID>` with plain `mkdir`, then write `{locked, pr-context.json{pr:int}}` BEFORE the fanout in the checkout toplevel; explicit collisions fail and internal IDs use bounded cryptographic-discriminator retry. Setup emits one closed identity/digest receipt and installs no `EXIT` trap. Explicit setup failures abandon through that receipt; finalization revalidates it, publishes the verdict exact-name/no-clobber, then removes only the two markers. mtime = THIS run's write time and stale interruption remains freshness-bounded. |
