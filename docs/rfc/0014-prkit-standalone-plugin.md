...
### 5.3 Copy manifest (the PR-phase subgraph — 37 files)
...
### 14.1 Approach
UberDev's Codex tree is already Codex-adapted (commands→`uberdev-cmd-*` command-skills, agents→TOML via `codex/tools/convert-agents.py`, paths fixed to `~/.codex`/`CODEX_HOME`/`.codex-plugin`). So prkit's Codex port is extracted **from `codex/uberdev-codex/`** (not re-transformed from scratch): a second manifest (`tools/prkit/manifest-codex.txt`, count-locked at **56**) drives a copy stage that rewrites both the **destination path** and the **content** with `uberdev`→`prkit`. The existing blanket rewrite already handles every Codex-specific literal — `uberdev-codex`→`prkit-codex`, `uberdev-cmd-`→`prkit-cmd-`, the TOML agent names, the `uberdev-codex-primer` sentinel.
