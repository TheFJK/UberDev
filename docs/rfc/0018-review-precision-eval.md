...
| Baseline | `tools/eval/precision-baseline.json` | Per-lens counts + the prompt-surface digests the numbers were measured against. |
...
re-affirmation deferral needs a stated reason. A `pending` entry naming a path
that has **not** drifted also fails the check, so the list cannot accumulate
