...
- **D220b** — Locked-marker contract at `.uberdev/runs/<RUN_ID>/locked` + sibling `pr-context.json {pr, issue, started_at}`. Writer: leaf `/uberdev:review-pr` at start of post-impl fanout (`commands/review-pr.md` Step 4); EXIT trap removes both on exit. Reader: `/goal` Phase 2b via `_uberdev_goal_locked_marker_for_pr_fresh "$pr_num" "$REVIEW_GRACE_SECS"`. Stale-marker safety net: the grace-window check bounds staleness even on writer SIGKILL (no operator intervention required).
