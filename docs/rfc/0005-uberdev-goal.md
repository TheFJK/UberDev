...
|------|-------------------------|----------|
| D2  | Issue state machine: six states `input → dispatched → solving → pr-pushed → resolved`; `dispatched → failed`, `solving → failed`, and `pr-pushed → failed` allowed; no audit event on issue transitions (derived state only). `dispatched` (issue #236) is the pre-spawn guard the parent writes BEFORE `uberdev_dispatch_one` so any leaf-side failure between spawn and the post-spawn `solving` write still leaves a TSV row the Phase-1 skip-check (`dispatched\|solving\|pr-pushed`) can match — closing the silent double-spawn surface where a pre-state-write leaf crash looked identical to "never attempted". **This row is amended twice and the amendments govern**: `resolved-by-no-action` (D249a-state) makes it seven states, and `pr-pushed → input` (D592a-arc) adds the one backwards arc, so `pr-pushed` is no longer terminal. | §3.2.2, D249a, D592a |
...
### D592a — partial-chain re-entry vocabulary (issue #592, 2026-08-17)

Addendum row for the two **closed vocabularies** widened so a PR whose solver task chain stopped short can be told apart from a converged one. Bug-fix scope under §2.3: no new RFC. Both vocabularies are declared closed and require an amendment here before any code widens them — `GOAL_AUDIT_EVENT_ENUM` by D3 (`lib/goal-abort.sh` restates the rule at its refusal to invent a member), the issue machine's arc list by D2. This row is that amendment, and it is deliberately narrow: it widens the two vocabularies and nothing else. The mechanism that consumes them (the ledger, the arming decision, the Phase-3 collection and the driver-side carrier) is behaviour, not vocabulary, and needs no RFC row.

- **D592a-event** — `goal_partial_chain` audit event. Payload: `{issue, pr, stage}` with `stage ∈ {requeued, at_cap, transition_refused}` plus the diagnostic no-op stage for a decision that reached the helper and changed nothing. Every stage is a **decision record**, which is the point: without one, a run that declined to re-queue is indistinguishable from a run that had nothing to re-queue — the same collapse the whole issue is about, one layer up. Reaching the per-issue re-queue cap is **not** a halt, so `GOAL_CIRCUIT_BREAKER_REASONS` (closed at 9 members when this row was written; 10 since D624a) gains nothing. **This member is RESERVED: no production code emits it.** The re-queue mechanism it records is tracked in issue **#613**; the name is authorized here so the closed enum is widened once rather than twice. What ships today is the opposite — `lib/goal-phase3.sh` halts convergence and states that a landed partial delivery is permanent for the run.
- **D592a-delivery** — `goal_partial_delivery` audit event. Payload: `{goal_id, pr, issue, cycle}`, emitted by `lib/goal-watch.sh` as a partial-chain PR leaves `green` for `merging`, on **both** arms that make that transition (the already-merged/resume arm and the dispatch-success arm) and on neither arm that withholds the merge. This is the member of the pair that **has a production emitter**, and it is what stops an unattended run from reporting a converged issue it did not converge. One row per PR is conditional on the `MERGING` sentinel write succeeding; a run whose sentinel write fails re-flags the PR each tick, which is accepted rather than de-duplicated because such a run is already degraded and says so on stderr.

  Together these two members extend `GOAL_AUDIT_EVENT_ENUM` **13 → 15**. `skills/goal-pipeline/SKILL.md` and the `G11` block in `tests/goal.test.sh` both state 15; an amendment naming only one of the two would leave the member that actually ships authorized by nothing.
- **D592a-arc** — `pr-pushed → input` issue transition. The **only** backwards arc in the machine, and the reason `pr-pushed` is no longer terminal. Like `D592a-event`, the arc is **authorized and accepted by the machine but not yet traversed by any caller** — it is vocabulary landed ahead of the re-queue mechanism in issue #613. A partial delivery still lands its PR, so the issue must re-enter the queue for the tasks that never shipped; Phase 1 then re-claims it through the existing `input → dispatched` path. Modelled as an arc rather than a new state precisely so `GOAL_ISSUE_STATE_ENUM` stays byte-identical at 7 members and every existing reader of that scalar is unaffected. `solving → input` stays **rejected** (rc=2): what is modelled is re-entry after a landed partial delivery, never a slip backwards out of an in-flight solve. `resolved`, `resolved-by-no-action` and the three `→ failed` sinks remain hard terminal, so D249a-skip's control-flow terminality is untouched for those states.

### D624a — `backend_resolve_failed` circuit-breaker reason (issue #624, 2026-08-19)

Amendment row widening `GOAL_CIRCUIT_BREAKER_REASONS` **9 → 10**. `skills/goal-pipeline/SKILL.md` declares that set CLOSED and mandates an RFC amendment before any code adds a member; this row is that amendment, and like D592a it is deliberately narrow — it authorises one member and changes nothing else about the loop.

- **D624a-reason** — `backend_resolve_failed` circuit-breaker reason. Payload: `{reason, step: "phase3_rehydrate", backend, exit_code}`. Emitted from the last line of `lib/goal-phase3.sh`'s `rehydrate` region, when `uberdev_dispatch_resolve_env` returns non-zero. `backend` is shape-gated to lowercase letters before interpolation (the audit payload is hand-assembled JSON and `uberdev_goal_audit` escapes nothing); anything else — including an empty value — is recorded as `unknown`. Deliberately a SHAPE gate rather than a copy of `lib/dispatch.sh`'s backend enum: re-listing that vocabulary inside `lib/goal-phase3.sh` would plant an unmarked mirror of a contract the file does not own (RFC 0016 §2).

  **Why a tenth member and not a `phase` subfield.** Both existing subfield reuses discriminate two shapes of the *same* failure — `solver_failed` + `phase=partial_chain` (D592a) and `stuck_loop` + `phase=blocks_cycle` — so the reason string stays a truthful answer to "what failed" in either arm. A host that ships neither `timeout(1)` nor `gtimeout(1)` is not a solver that fell short and is not a loop that stalled; hanging it off either reason would make the reason string a lie for readers that match on it alone, which is exactly what `skills/goal-pipeline/workflow.js` does (it publishes `haltReason` from the reason and `haltPhase` separately).

  **Why the halt needed a row at all.** The exit already existed; it just wrote nothing. The collect relay reports the `.payload.reason` of the **last** `goal_circuit_breaker` row in `goal-<id>.jsonl`, so a halt that appends no row is published under whatever reason an earlier phase — or, on a `--resume`, an earlier **cycle** — last wrote. `lib/goal-phase0.sh` does not truncate the jsonl on a resume, so that stale row is routinely present. Before #592's relay fix this failed *blank*; after it, it failed with a specific and plausible wrong cause, which is the worse of the two. The invariant this row locks is therefore not "add a reason" but **every `exit 1` in `lib/goal-phase3.sh` writes the breaker row that explains it**, and the file's EXIT CONTRACT header now states it.

  **Reachability, stated honestly.** `uberdev_dispatch_resolve_env` has exactly one non-zero return (the `TIMEOUT_BIN` probe) and returns 0 early for `backend=workflow`, which RFC 0015 made `/goal`'s default. The path is therefore reachable only under `--backend=background` or `--backend=wezterm` on a host with no `timeout(1)`. Narrow — but a mis-attributed halt reason costs an operator the whole diagnosis, and the correction is one audit row.

  **Surfaces moved together** (a count left behind is how a closed set stops being closed): the enum scalar and its prose in `skills/goal-pipeline/SKILL.md`; the `CIRCUIT_BREAKER_HALT` rehydration allowlist in `lib/goal-state.sh` (which keeps its ONE declared divergence, `-solver_failed`, rather than growing a second); the breaker-family arithmetic in `tests/contract_markers.py`; the count in `commands/goal.md` (count only — that file is marker-policed against re-listing the enum); the table row and prose in `docs/rfc/0016-contract-markers.md`; and the "closed at 9 members" claim in D592a above, which this row supersedes.

- **D590a** — `goal.implement_budget` config key + `UBERDEV_GOAL_IMPLEMENT_BUDGET` env + `--implement-budget=N` CLI arg. Range `[4, 96]`, default `24` — the FLEET's clamp triple, not a second one, because the value's only meaning is the number `skills/solve-fleet/workflow.js` will run its CB3 counter against. Resolution precedent mirrors `goal.max_cycles`, with **one extra tier below the config key**: `UBERDEV_SOLVE_FLEET_IMPLEMENT_BUDGET`, the variable `lib/solve-launcher.sh` already reads when it composes the fleet envelope. That tier is the whole point of the key — it is what makes `/goal`'s pre-claim CB1 projection track the budget the nested fleet actually runs with, rather than a second knob that happens to share a name.

  **Why a knob was needed at all.** `projectedAgentsForCycle` froze the per-issue cost at a literal `33`, correct only at the fleet default. The fleet honours a raised `UBERDEV_SOLVE_FLEET_IMPLEMENT_BUDGET` inside a `/goal` run (it inherits it through the Phase-1 launcher call), so at the clamp ceiling one design-tier issue really costs 105 — a 72-agent under-count per issue. CB1 is the only NAMED halt in the pipeline, so an under-projection does not mis-report: the breaker stops binding, and the run dies against the runtime's own agent lifetime cap with no halt reason, no audit row and no cycle record (issue #590).

  **Both directions, or the knob is a lie.** Phase 0 publishes the resolved value as the envelope's `implementBudget` key, and the driver's claim relay pins it back onto the STEP 2 launcher command line. Without the second half, `--implement-budget=48` would change what `/goal` projects and not what the fleet spends — an asymmetry pointed the other way, and one that no test could distinguish from the fix.

  **Surfaces moved together**: the default constant in `skills/goal-pipeline/SKILL.md`'s Constants block and its runtime mirror in `lib/goal-state.sh`; the CLI arm, the range read and the envelope key in `lib/goal-phase0.sh`; the `implementBudget` clamp, `perIssueCostAtBudget`, `fleetRunCost` and the claim-relay pin in `skills/goal-pipeline/workflow.js`; the flag docs in `commands/goal.md`; the CB1 rows in RFC 0015 §4.2.
