...
- **D592a-event** — `goal_partial_chain` audit event (`GOAL_AUDIT_EVENT_ENUM` extended 13 → 14). Payload: `{issue, pr, stage}` with `stage ∈ {requeued, at_cap, transition_refused}` plus the diagnostic no-op stage for a decision that reached the helper and changed nothing. Every stage is a **decision record**, which is the point: without one, a run that declined to re-queue is indistinguishable from a run that had nothing to re-queue — the same collapse the whole issue is about, one layer up. Reaching the per-issue re-queue cap is **not** a halt: the run continues and the flag surfaces in the run summary, so `GOAL_CIRCUIT_BREAKER_REASONS` (closed at 9 members) gains nothing.
- **D592a-arc** — `pr-pushed → input` issue transition. The **only** backwards arc in the machine, and the reason `pr-pushed` is no longer terminal. A partial delivery still lands its PR, so the issue must re-enter the queue for the tasks that never shipped; Phase 1 then re-claims it through the existing `input → dispatched` path. Modelled as an arc rather than a new state precisely so `GOAL_ISSUE_STATE_ENUM` stays byte-identical at 7 members and every existing reader of that scalar is unaffected. `solving → input` stays **rejected** (rc=2): what is modelled is re-entry after a landed partial delivery, never a slip backwards out of an in-flight solve. `resolved`, `resolved-by-no-action` and the three `→ failed` sinks remain hard terminal, so D249a-skip's control-flow terminality is untouched for those states.
