...
**Post-audit amendment (#430, 2026-08-10):** `systematic-debugging/find-polluter.sh` keeps the audit-wide base `e7a2d16` — the SHA `plugins/uberdev/vendor.json` records for the whole `skills/systematic-debugging` component — and has since **adopted the enumeration fix from upstream v6.2.0** (`3dcbd5c4b48e02263fbf4a3c01e3fe4f81d584d9`), that hunk only, plus **two** local additions — (1) the fail-loud exit-2 refusals to render a verdict the run cannot back, and (2) whitespace- and glob-safe enumeration (upstream iterates the unquoted match string and takes the count from its line count; this copy reads the matches into an array and counts the array) — so **4** files now carry expected-DIFFER status and **16** remain byte-equivalent to the SHA pinned in their own provenance header. Both additions must be re-applied on a re-sync: taking upstream at the pinned SHA and re-applying only the refusals silently drops the enumeration fix that `tests/find-polluter.test.sh` F12 exists to lock. Row 15 and the allowlist below are updated accordingly. The audit-wide `FRESH_SHA`/`e7a2d16` **still applies** to this file as its base — it is deliberately *not* re-baselined to `3dcbd5c`, because the other 10 files in the component were never re-copied and at least `CREATION-LOG.md` and `root-cause-tracing.md` provably differ between the two upstream commits. Re-baselining the component is issue #462's job, not #430's.
...
| 15 | `systematic-debugging/find-polluter.sh` | `RECOMPUTE` | 1986 | `RECOMPUTE` | `dd7b8f13c4cc` | DIFFER (expected: base `e7a2d16` + v6.2.0 enumeration fix adopted + local exit-2 refusals + whitespace/glob-safe enumeration — see #430). Local columns carry the `RECOMPUTE` sentinel declared above the table. |
...
#   systematic-debugging/find-polluter.sh — base e7a2d16 + v6.2.0 enumeration fix adopted + TWO local additions (#430): (1) an exit-2 refusal
...
#     for the component), and 3dcbd5c4b48e02263fbf4a3c01e3fe4f81d584d9 is the commit the enumeration hunk alone was
#     taken from. Diff it at e7a2d16 to see every local change; the v6.2.0 enumeration hunk will be among them.
