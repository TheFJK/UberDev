...
echo "== P21: the fixer-wave return contract names blocked_on_file (#755) =="
...
P21_PROMPT="$(awk '
  ...
  infield { print }
' "$SKILL")"
P21_PROMPT_LINES="$(grep -c . <<<"$P21_PROMPT")"
if [ "$P21_PROMPT_LINES" -ge 6 ]; then
  echo "  PASS  P21: the §2a return block extracted $P21_PROMPT_LINES lines"; PASS=$((PASS + 1))
else
  echo "  FAIL  P21: the §2a return block extracted $P21_PROMPT_LINES lines — every row below is vacuous"; FAIL=$((FAIL + 1))
fi
assert_in "$P21_PROMPT" 'blocked_on_file:' "P21: the return contract declares the field"
assert_in "$P21_PROMPT" '- file:' "P21: and it carries a path"
assert_in "$P21_PROMPT" 'reason:' "P21: and a reason"
...
P21_RULES="$(awk '
  ...
  inrules { print }
' "$SKILL")"
P21_RULE_LINES="$(grep -c . <<<"$P21_RULES")"
if [ "$P21_RULE_LINES" -ge 8 ]; then
  echo "  PASS  P21: the §2a rules block extracted $P21_RULE_LINES lines"; PASS=$((PASS + 1))
else
  echo "  FAIL  P21: the §2a rules block extracted $P21_RULE_LINES lines — the rows below are vacuous"; FAIL=$((FAIL + 1))
fi
assert_in "$P21_RULES" 'status: REFUSED' "P21: the REFUSED emission case is stated in the rules"
assert_in "$P21_RULES" 'status: APPLIED' "P21: the APPLIED emission case is stated in the rules"
assert_in "$P21_RULES" 'blocked_on_file' "P21: both cases name the field the agent must emit"
assert_in "$P21_RULES" 'never a line range' "P21: the reason is carried, the coordinates are not"

echo "== P21b: the allowed set still derives ONLY from the wave plan (#755) =="
...
#
# The two bypass patterns are HOISTED into variables because P21c drives the
...
P21B_BYPASS_RE='blocked.on'
P21B_APPEND_RE='PREMERGE_ASSIGNED_LIST"$'
assert_in "$FIX_FENCE" 'jq -r '"'"'.waves[][].file'"'"' <"$PREMERGE_WAVES_FILE" >"$PREMERGE_SCOPE_RAW" || exit 2' \
  "P21b: the allowed set is still derived from the wave plan, unchanged"
assert_not_in "$FIX_FENCE" "$P21B_BYPASS_RE" \
  "P21b: and the fix-commit fence names no blocked-on artifact"
assert_not_in "$FIX_FENCE" "$P21B_APPEND_RE" \
  "P21b: nothing appends to the assigned list after it is built"
...
P21B_PRODUCERS="$(grep -cE '>+[[:space:]]*"\$PREMERGE_ASSIGNED_LIST"' <<<"$FIX_FENCE")"
if [ "$P21B_PRODUCERS" = "1" ]; then
  echo "  PASS  P21b: the assigned list has exactly one producer"; PASS=$((PASS + 1))
else
  echo "  FAIL  P21b: the assigned list has $P21B_PRODUCERS producers, wanted 1"; FAIL=$((FAIL + 1))
fi

echo "== P21c: the widening is declared, bounded and non-accumulating (#755) =="
assert_fixed "$FENCES" '--carry-blocked' "P21c: a fence turns the widening on"
assert_fixed "$FENCES" '--repo-root "$PREMERGE_ROOT"' "P21c: and hands it a root to contain against"
assert_fixed "$SKILL" 'PREMERGE_MAX_BLOCKED_ON' "P21c: the SKILL declares the cap"
assert_fixed "$LIB" 'MAX_BLOCKED_ON' "P21c: the library declares the cap it enforces"
assert_fixed "$LIB" 'blocked-on-%02d.json' "P21c: the library reads ONE predecessor by name, not a range"
assert_fixed "$LIB" '_beneath_repo_root' "P21c: containment is a filesystem check the library actually has"
assert_fixed "$SKILL" 'blocked-on-<NN>.json' "P21c: the SKILL names the artifact the controller writes"
assert_fixed "$SKILL" 'BLOCKED=<n>' "P21c: the triage line documents the new field"
assert_grep "$SKILL_MISTAKES" 'Adding a `blocked_on_file` path straight to the commit fence' \
  "P21c: the bypass is written down as a mistake, not left to be rediscovered"
assert_grep "$SKILL_MISTAKES" 'Unioning `blocked_on_file` across every earlier attempt' \
  "P21c: and so is the accumulating union"
# ANTI-VACUITY for the two assert_not_in rows above: the SAME two patterns —
# read out of $P21B_BYPASS_RE and $P21B_APPEND_RE, never re-typed — over a body
...
P21C_MUTANT='PREMERGE_ASSIGNED_LIST="$PREMERGE_RUN_DIR/x.assigned"
jq -r ".blocked_on[].file" <"$PREMERGE_RUN_DIR/blocked-on-01.json" >>"$PREMERGE_ASSIGNED_LIST"'
if grep -qE -e "$P21B_BYPASS_RE" <<<"$P21C_MUTANT"; then
  echo "  PASS  P21c: the bypass detector sees a bypass when there is one"; PASS=$((PASS + 1))
else
  echo "  FAIL  P21c: the bypass detector cannot see a bypass — P21b is vacuous"; FAIL=$((FAIL + 1))
fi
if grep -qE -e "$P21B_APPEND_RE" <<<"$P21C_MUTANT"; then
  echo "  PASS  P21c: the append detector sees an append when there is one"; PASS=$((PASS + 1))
else
  echo "  FAIL  P21c: the append detector cannot see an append — P21b is vacuous"; FAIL=$((FAIL + 1))
fi
