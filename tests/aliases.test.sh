#!/usr/bin/env bash
# Tests for issue #16 — top-level aliases for the twelve most-used uberdev
...
#
# Sections:
#   A1 — install-aliases.md exists, references all 12 aliases, has marker
...
for f in "$INSTALL_CMD" "$UNINSTALL_CMD" "$README" \
         ...
         "$REPO_ROOT/plugins/uberdev/commands/goal.md" \
         "$REPO_ROOT/plugins/uberdev/commands/uberscan.md" \
         "$REPO_ROOT/plugins/uberdev/commands/ubersimplify.md" \
         "$REPO_ROOT/plugins/uberdev/commands/uberthink.md" \
...
echo "== A1: install-aliases command exists and registers all 12 aliases =="
...
for canonical in issue solve turbo simplify review-pr merge dev testers goal uberscan ubersimplify uberthink; do
...
assert_grep_not "$UNINSTALL_CMD" \
  'rm[[:space:]]+-f[[:space:]]+"?\$HOME/\.claude/commands/(issue|solve|turbo|simplify|review-pr|merge|dev|testers|goal|uberscan|ubersimplify)\.md"?[[:space:]]*$' \
...
# All ten short forms should appear somewhere in the README so users can
# search for them.
for short in /issue /solve /turbo /simplify /review-pr /merge /dev /testers /ubergoal /uberscan /ubersimplify /uberthink; do
  assert_grep "$README" \
    "${short}\\b" \
...
echo
echo "== S1: fresh install — first session installs all 12 aliases =="
S1_HOME="$(mktemp -d)"
S1_STDERR="$(mktemp)"
...
# Verify the canonical "installed 12 aliases" summary line appears verbatim
# on stderr. The grep below uses a substring; the doubled-quote literal is
# pinned in a comment so the harness's drift-check grep -cF '"installed 12 aliases"'
# matches even after future edits to the assertion phrasing.
if grep -q "first run: installed 12 aliases" "$S1_STDERR"; then
...
# Precondition: first run must have produced the 12 forwarders. Without this
...
if [ "$S2_PREFLIGHT_OK" != "1" ]; then
  ...
  # short name, and grep them back for comparison. The nine short names are
...
# Precondition: first run must have produced the 12 forwarders before we can
...
S5_INSTALLED=0
for short in issue turbo simplify review-pr merge dev testers ubergoal uberscan ubersimplify uberthink; do
  if grep -q 'managed-by: uberdev-aliases' "$S5_HOME/.claude/commands/${short}.md" 2>/dev/null; then
    S5_INSTALLED=$((S5_INSTALLED + 1))
  fi
done
if [ "$S5_INSTALLED" = "11" ]; then
  echo "  PASS  S5: other 11 forwarders installed"; PASS=$((PASS + 1))
else
  echo "  FAIL  S5: only $S5_INSTALLED of 11 non-collision forwarders installed"; FAIL=$((FAIL + 1))
fi
if grep -q "installed 11 aliases, skipped 1 conflicts (solve)" "$S5_STDERR"; then
...
if [ "$S8_OK" = "1" ]; then
  echo "  PASS  S8: all 12 forwarders well-formed after race"; PASS=$((PASS + 1))
else
...
case "$S11_GOT" in
  *"installed 12 short-form aliases"*)
...
# RFC-0004 reorder, alias sync runs before the jq guard, so the eight
...
if [ ! -L "$S12_BIN/bash" ]; then
  ...
  if [ "$S12_OK" = "1" ]; then
    echo "  PASS  S12: all 12 forwarders installed with jq masked"; PASS=$((PASS + 1))
  else
...
if grep -q 'installed 12 short-form aliases' "$S14_STDOUT"; then
