...
assert_grep_not "$UNINSTALL_CMD" \
  'rm[[:space:]]+-f[[:space:]]+"?\$HOME/\.claude/commands/(issue|solve|turbo|simplify|review-pr|merge|dev|testers|goal|uberscan|ubersimplify|uberthink|cluster|ubercluster)\.md"?[[:space:]]*$' \
...
# search for them.
for short in /issue /solve /turbo /simplify /review-pr /merge /dev /testers /ubergoal /uberscan /ubersimplify /uberthink /ubercluster; do
  assert_grep "$README" \
    "${short}\\b" \
...
S5_INSTALLED=0
for short in issue turbo simplify review-pr merge dev testers ubergoal uberscan ubersimplify uberthink ubercluster; do
  if grep -q 'managed-by: uberdev-aliases' "$S5_HOME/.claude/commands/${short}.md" 2>/dev/null; then
    S5_INSTALLED=$((S5_INSTALLED + 1))
  fi
done
if [ "$S5_INSTALLED" = "12" ]; then
  echo "  PASS  S5: other 12 forwarders installed"; PASS=$((PASS + 1))
else
