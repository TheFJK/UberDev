...
for f in "$INSTALL_CMD" "$UNINSTALL_CMD" "$README" \
         ...
         "$REPO_ROOT/plugins/uberdev/commands/cluster.md" \
...
# Belt-and-braces: uninstall must NOT do an unguarded rm of any forwarder path.
assert_grep_not "$UNINSTALL_CMD" \
  'rm[[:space:]]+-f[[:space:]]+"?\$HOME/\.claude/commands/(issue|solve|turbo|simplify|review-pr|merge|premerge|dev|testers|goal|uberscan|ubersimplify|uberthink|cluster|ubercluster)\.md"?[[:space:]]*$' \
...
# search for them.
for short in /issue /solve /turbo /simplify /review-pr /merge /premerge /dev /testers /ubergoal /uberscan /ubersimplify /uberthink /ubercluster; do
  assert_grep "$README" \
    "${short}\\b" \
...
echo
echo "== S1: fresh install — first session installs all 15 aliases =="
S1_HOME="$(mktemp -d)"
S1_STDERR="$(mktemp)"
...
# Precondition: first run must have produced the 15 forwarders. Without this
...
# Precondition: first run must have produced the 15 forwarders before we can
...
S5_INSTALLED=0
for short in issue turbo simplify review-pr merge premerge dev testers ubergoal uberscan ubersimplify uberthink ubercluster; do
  if grep -q 'managed-by: uberdev-aliases' "$S5_HOME/.claude/commands/${short}.md" 2>/dev/null; then
    S5_INSTALLED=$((S5_INSTALLED + 1))
  fi
done
if [ "$S5_INSTALLED" = "13" ]; then
  echo "  PASS  S5: other 13 forwarders installed"; PASS=$((PASS + 1))
else
  echo "  FAIL  S5: only $S5_INSTALLED of 13 non-collision forwarders installed"; FAIL=$((FAIL + 1))
fi
# 15 total - 1 conflict (solve) = 14 installed; use [0-9]+ to keep this
...
if [ "$S8_OK" = "1" ]; then
  echo "  PASS  S8: all 15 forwarders well-formed after race"; PASS=$((PASS + 1))
else
...
if [ ! -L "$S12_BIN/bash" ]; then
  ...
  if [ "$S12_OK" = "1" ]; then
    echo "  PASS  S12: all 15 forwarders installed with jq masked"; PASS=$((PASS + 1))
  else
