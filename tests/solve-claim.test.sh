...
echo "== Version bump 0.34.5 -> 0.34.6 propagated =="
assert_grep "$PLUGIN_JSON" \
  '"version": "0.34.6"' \
  "plugin.json bumped to 0.34.6"
assert_grep "$MARKETPLACE_JSON" \
  '"version": "0.34.6"' \
  "marketplace.json bumped to 0.34.6"
assert_grep "$README" \
  "version-0\\.34\\.6-blue" \
  "README version badge bumped to 0.34.6"
assert_grep "$CHANGELOG" \
  '^## \[0\.34\.6\]' \
  "CHANGELOG has [0.34.6] section header"
