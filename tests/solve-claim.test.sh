...
echo "== Version bump 0.34.9 -> 0.34.10 propagated =="
assert_grep "$PLUGIN_JSON" \
  '"version": "0.34.10"' \
  "plugin.json bumped to 0.34.10"
assert_grep "$MARKETPLACE_JSON" \
  '"version": "0.34.10"' \
  "marketplace.json bumped to 0.34.10"
assert_grep "$README" \
  "version-0\\.34\\.10-blue" \
  "README version badge bumped to 0.34.10"
assert_grep "$CHANGELOG" \
  '^## \[0\.34\.10\]' \
  "CHANGELOG has [0.34.10] section header"
