...
echo "== Version bump 0.33.16 -> 0.33.17 propagated =="
assert_grep "$PLUGIN_JSON" \
  '"version": "0.33.17"' \
  "plugin.json bumped to 0.33.17"
assert_grep "$MARKETPLACE_JSON" \
  '"version": "0.33.17"' \
  "marketplace.json bumped to 0.33.17"
assert_grep "$README" \
  "version-0\\.33\\.17-blue" \
  "README version badge bumped to 0.33.17"
assert_grep "$CHANGELOG" \
  '^## \[0\.33\.17\]' \
  "CHANGELOG has [0.33.17] section header"
