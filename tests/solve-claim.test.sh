...
echo "== Version bump 0.34.10 -> 0.34.11 propagated =="
assert_grep "$PLUGIN_JSON" \
  '"version": "0.34.11"' \
  "plugin.json bumped to 0.34.11"
assert_grep "$MARKETPLACE_JSON" \
  '"version": "0.34.11"' \
  "marketplace.json bumped to 0.34.11"
assert_grep "$README" \
  "version-0\\.34\\.11-blue" \
  "README version badge bumped to 0.34.11"
assert_grep "$CHANGELOG" \
  '^## \[0\.34\.11\]' \
  "CHANGELOG has [0.34.11] section header"
