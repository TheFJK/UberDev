...
ck "dispatches 3 code-simplifier lenses" "grep -q 'code-simplifier' '$SKILL' && grep -q 'Reuse' '$SKILL' && grep -q 'Quality' '$SKILL' && grep -q 'Efficiency' '$SKILL'"
...
ck "config namespace ubersimplify.*" "grep -q 'ubersimplify.max_chunks' '$SKILL' && grep -q 'fanout_concurrency.ubersimplify' '$SKILL'"
