...
WAIVERS="$(cat <<'EOF'
premerge-pipeline	2709	~53% fenced (1434/2709; re-measured — the counter that reproduces the seven never-raised rows exactly, and under which neither the old 1461 here nor merge-pipeline 546 reproduces); rose 2486->2664 then 2664->2709, both for #716 — the first raise added the two `uberdev-executable` fences the orchestrator env-prefixes and RUNS, one to decide the premerge trust trail and one to publish it; the second hardens that publication fence in place: a numeric guard on `combined_pr` before the first write, `git reset --soft` unwind arms on the two refusals that still precede a successful push, and a `half_emitted` state (`label_unprovisioned` / `label_unapplied`) on the two post-push `gh` arms, plus the `### 5b` summary row that transcribes that state — executable-fence code with its inline rationale, and the report contract the orchestrator prints, both obeyed at runtime, so neither can move to references/
merge-pipeline	1919	~28% fenced (546/1919), no references/ yet — prose extraction is open work; rose 1778->1835 then 1835->1919, both for #716's generalisation of Phase 1.4 PATH_2 over the trust instrument — the second raise is sub-condition (a) resolving LABEL_INSTRUMENTS as a SET plus the (b)/(b.5) bindings that read that set, which like the first raise is the gate's own decision procedure, read and acted on at runtime, so it cannot move to references/
