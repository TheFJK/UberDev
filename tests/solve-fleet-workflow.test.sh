...
(async function () {
  ...
  out.bSolverNoCite     = !!(solverB && solverB.prompt.indexOf("from CLAUDE.md") < 0);
  out.bSolverHonest     = !!(solverB && solverB.prompt.indexOf("not a quotation of any file") >= 0);
  out.bSolverReadsRepo  = !!(solverB && solverB.prompt.indexOf("the root of YOUR worktree") >= 0);
  ...
  out.bSolverVersionLock = !!(solverB && solverB.prompt.indexOf("Do NOT bump the project version") >= 0);
