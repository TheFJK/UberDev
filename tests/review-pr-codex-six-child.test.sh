...
# The evidence gate's failure class is an assertion target below, so capture the
# boundary's diagnostics instead of letting them stream straight past. They are
# echoed back verbatim so a failing run still reads identically.
...
# A provider this fixture did not deliberately hang must never end in
# `timed_out`. That state means the harness's own wall-clock wait budget
# expired while the provider was still settling — a fixture-budget failure, not
# missing reviewer evidence. It has to announce itself as its own class,
# because it otherwise surfaces downstream only as a suppressed aggregate and
# reads exactly like a genuine evidence defect (#365).
...
if ! assert_no_unbudgeted_provider_timeout; then
  dump_child_terminal_states
  dump_git_mutation_diagnostics
  exit 1
fi
...
SIX_CHILD_PROVIDER_SETTLE_BUDGET=60
